...
class TestSearchContentDescription:
    ...
    def test_token_cost_comparison_table(self):
        ...
        for format_name, token_range, description_key in required_entries:
            ...
            assert (
                token_range in self.description
            ), f"Missing token estimate: {token_range}"
            # Check that description context is present
            assert (
                description_key.lower() in self.description.lower()
    ...
    def test_efficiency_ordering(self):
        ...
        comparison_section_start = self.description.find(
            "💡 TOKEN EFFICIENCY COMPARISON"
        )
        comparison_section = self.description[
            comparison_section_start : comparison_section_start + 1000
        ]
    ...
    def test_guidance_actionability(self):
        ...
        assert (
            action_found >= 5
        ), f"Insufficient actionable guidance. Found only {action_found} action words"
    ...
    def test_no_redundant_information(self):
        ...
        for phrase in key_phrases:
            ...
            assert (
                3 <= count <= 20
            ), f"Phrase '{phrase}' mentioned {count} times (should be 3-20)"
    ...
    def test_compatibility_with_existing_features(self):
        ...
        for param in mentioned_params:
            assert (
                param in properties
            ), f"Description mentions non-existent parameter: {param}"
        ...
        for param in mentioned_params:
            ...
            assert (
                param_type == "boolean"
            ), f"Parameter {param} should be boolean, got {param_type}"
