...
""")
    ...
    def test_basic_cli_execution_python(self, sample_files):
        ...
        result = subprocess.run([
            ...
            "--query-key", "function", sample_files[0]
    ...
    def test_basic_cli_execution_javascript(self, sample_javascript_file):
        ...
        result = subprocess.run([
            sys.executable, "-m", "tree_sitter_analyzer",
            "--query-key", "function", sample_javascript_file
        ], capture_output=True, text=True, timeout=30)
    ...
    def test_class_query_execution(self, sample_files):
        ...
        result = subprocess.run([
            sys.executable, "-m", "tree_sitter_analyzer",
            "--query-key", "class", sample_files[1]
        ], capture_output=True, text=True, timeout=30)
    ...
    def test_multiple_file_processing(self, sample_files):
        ...
        for i, file_path in enumerate(sample_files):
            result = subprocess.run([
                sys.executable, "-m", "tree_sitter_analyzer",
                "--query-key", "function", file_path
            ], capture_output=True, text=True, timeout=30)
            
            assert result.returncode == 0, f"CLI failed for file {i} with stderr: {result.stderr}"
    ...
    def test_output_format_json(self, sample_files):
        ...
        result = subprocess.run([
            sys.executable, "-m", "tree_sitter_analyzer",
            "--query-key", "function", "--output-format", "json", sample_files[0]
        ], capture_output=True, text=True, timeout=30)
    ...
    def test_output_format_text(self, sample_files):
        ...
        result = subprocess.run([
            sys.executable, "-m", "tree_sitter_analyzer",
            "--query-key", "function", "--output-format", "text", sample_files[0]
        ], capture_output=True, text=True, timeout=30)
    ...
    def test_custom_query_string(self, sample_files):
        ...
        result = subprocess.run([
            sys.executable, "-m", "tree_sitter_analyzer",
            "--query-string", "(function_definition name: (identifier) @function)", sample_files[0]
        ], capture_output=True, text=True, timeout=30)
    ...
    def test_filter_expression(self, sample_files):
        ...
        result = subprocess.run([
            sys.executable, "-m", "tree_sitter_analyzer",
            "--query-key", "function", "--filter", "name=function_a", sample_files[0]
        ], capture_output=True, text=True, timeout=30)
    ...
    def test_explicit_language_specification(self, sample_files):
        ...
        result = subprocess.run([
            sys.executable, "-m", "tree_sitter_analyzer",
            "--query-key", "function", "--language", "python", sample_files[0]
        ], capture_output=True, text=True, timeout=30)
    ...
    def test_error_cases_nonexistent_file(self):
        ...
        result = subprocess.run([
            sys.executable, "-m", "tree_sitter_analyzer",
            "--query-key", "function", "nonexistent_file.py"
        ], capture_output=True, text=True, timeout=30)
        ...
        assert any(keyword in error_msg for keyword in [
            "not exist", "not found", "no such file", "file not found"
    ...
    def test_error_cases_invalid_language(self, sample_files):
        ...
        result = subprocess.run([
            sys.executable, "-m", "tree_sitter_analyzer",
            "--query-key", "function", "--language", "invalid_language", sample_files[0]
        ], capture_output=True, text=True, timeout=30)
    ...
    def test_error_cases_invalid_query_key(self, sample_files):
        ...
        result = subprocess.run([
            sys.executable, "-m", "tree_sitter_analyzer",
            "--query-key", "invalid_query_key", sample_files[0]
        ], capture_output=True, text=True, timeout=30)
    ...
    def test_error_cases_malformed_query_string(self, sample_files):
        ...
        result = subprocess.run([
            sys.executable, "-m", "tree_sitter_analyzer",
            "--query-string", "((invalid query syntax", sample_files[0]
        ], capture_output=True, text=True, timeout=30)
    ...
    def test_concurrent_cli_execution(self, sample_files):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
            futures = [
                executor.submit(run_cli_query, file_path)
        ...
        for i, result in enumerate(results):
            assert result.returncode == 0, f"Concurrent CLI execution {i} failed with stderr: {result.stderr}"
            assert len(result.stdout) > 0, f"No output from concurrent CLI execution {i}"
