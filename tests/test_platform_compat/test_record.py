...
class TestRecordCLI:
    ...
    def test_main_success(self, tmp_path):
        ...
        with patch('sys.argv', ['record', '--output-dir', str(tmp_path)]):
            with patch('tree_sitter_analyzer.platform_compat.record.BehaviorRecorder') as MockRecorder:
    ...
    def test_main_failure_exits(self, tmp_path):
        ...
        with patch('sys.argv', ['record', '--output-dir', str(tmp_path)]):
            with patch('tree_sitter_analyzer.platform_compat.record.BehaviorRecorder') as MockRecorder:
                MockRecorder.return_value.record_all.side_effect = RuntimeError("Test error")
