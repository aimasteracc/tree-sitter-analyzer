...
class TestQueryService:
    ...
    async def test_execute_query_missing_parameters(self):
        ...
        with pytest.raises(ValueError, match="Must provide either query_key or query_string"):
    ...
    async def test_execute_query_file_not_found(self):
        ...
        with patch.object(self.service, '_read_file_async') as mock_read:
            ...
            with pytest.raises(FileNotFoundError):
                await self.service.execute_query("nonexistent.py", "python", query_key="methods")
        ...
        with patch.object(self.service, '_read_file_async') as mock_read:
            ...
            with patch.object(self.service.parser, 'parse_code') as mock_parse:
                ...
                with pytest.raises(Exception, match="Failed to parse file"):
                    await self.service.execute_query("test.py", "python", query_key="methods")
            ...
            with patch.object(self.service.parser, 'parse_code') as mock_parse:
                ...
                with patch('tree_sitter_analyzer.core.query_service.query_loader') as mock_loader:
                    ...
                    with patch('tree_sitter_analyzer.core.query_service.TreeSitterQueryCompat.safe_execute_query') as mock_execute:
                        ...
                        # Mock filter
                        with patch.object(self.service.filter, 'filter_results') as mock_filter:
            ...
            with patch.object(self.service.parser, 'parse_code') as mock_parse:
                ...
                with patch('tree_sitter_analyzer.core.query_service.query_loader') as mock_loader:
                    ...
                    with pytest.raises(ValueError, match="Query 'unknown' not found"):
                        await self.service.execute_query("test.py", "python", query_key="unknown")
    ...
    def test_get_query_description(self):
        ...
        with patch('tree_sitter_analyzer.core.query_service.query_loader') as mock_loader:
            mock_loader.get_query_description.return_value = "Extract function definitions"
            
            result = self.service.get_query_description("python", "functions")
            
            assert result == "Extract function definitions"
            mock_loader.get_query_description.assert_called_once_with("python", "functions")
            ...
            with patch.object(self.service.parser, 'parse_code') as mock_parse:
                ...
                with patch('tree_sitter_analyzer.core.query_service.query_loader') as mock_loader:
                    ...
                    with patch('tree_sitter_analyzer.core.query_service.TreeSitterQueryCompat.safe_execute_query') as mock_execute:
                        ...
                        mock_node1.end_byte = 14    # "function1" ends at position 14 (includes full name)
                        ...
                        mock_node2.end_byte = 60    # "method1" ends at position 60 (includes full name)
