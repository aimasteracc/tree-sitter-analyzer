...
""")
    ...
    async def test_multiple_files_concurrent_performance(self, multiple_files):
        ...
        # 並行実行が効率的であることを確認
        efficiency = sequential_time / concurrent_time
        assert efficiency > 1.2, f"Multi-file concurrent execution not efficient: {efficiency:.2f}x"
        
        print(f"Multi-file Sequential: {sequential_time:.3f}s, Concurrent: {concurrent_time:.3f}s, Efficiency: {efficiency:.2f}x")
    ...
    async def test_memory_usage_monitoring(self, large_python_file):
        ...
        print(f"Memory increase: {memory_increase_percent:.2f}% ({memory_increase / 1024 / 1024:.2f} MB)")
    ...
    async def test_memory_usage_concurrent_execution(self, medium_python_file):
        ...
        memory_increase_percent = (memory_increase / initial_memory) * 100
        assert memory_increase_percent < 15.0, f"Concurrent memory increase too high: {memory_increase_percent:.2f}%"
        
        print(f"Concurrent memory increase: {memory_increase_percent:.2f}% ({memory_increase / 1024 / 1024:.2f} MB)")
    ...
    async def test_stress_test_high_concurrency(self, small_python_file):
        ...
        print(f"High concurrency: {successful_results}/{len(tasks)} successful in {duration:.3f}s")
    ...
    async def test_throughput_measurement(self, small_python_file):
        ...
        print(f"Throughput: {throughput:.2f} queries/sec ({completed_queries} queries in {actual_duration:.2f}s)")
    ...
    async def test_file_io_performance(self, large_python_file):
        ...
        if hasattr(service, '_read_file_async'):
            ...
            assert throughput_mbps > 1.0, f"File I/O too slow: {throughput_mbps:.2f} MB/s"
            
            print(f"File I/O performance: {throughput_mbps:.2f} MB/s ({file_size / 1024:.2f} KB in {duration:.3f}s)")