...
""")
    ...
    async def test_check_code_scale_error_handling(self, scale_tool):
        ...
        try:
            await scale_tool.execute({
                "file_path": "/invalid/path/outside/project.py"
            })
    ...
    async def test_set_project_path_error_handling(self, mcp_server):
        ...
        try:
            ...
            assert "directory" in str(e) or "not a directory" in str(e) or "file" in str(e)
        finally:
            # クリーンアップ
    ...
    async def test_error_message_quality(self, scale_tool, table_tool, query_tool):
        ...
        for tool, params, expected_error in test_cases:
            try:
                ...
                assert False, f"Expected {expected_error.__name__} for {tool.__class__.__name__}"
            except expected_error as e:
                error_msg = str(e)
                # エラーメッセージが空でないことを確認
                assert len(error_msg) > 0
                # ファイル名が含まれることを確認
                assert "nonexistent.py" in error_msg or "File" in error_msg or "file" in error_msg
                # 有用なキーワードが含まれることを確認
