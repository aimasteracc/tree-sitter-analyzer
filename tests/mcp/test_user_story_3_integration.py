...
class TestUserStory3Integration:
    ...
    def temp_project(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # 複雑なJavaファイル
            (project_root / "src" / "ComplexService.java").write_text("""package com.example.service;
...
""")
            ...
            # 複雑なTypeScriptファイル
            (project_root / "src" / "DataManager.ts").write_text("""import { EventEmitter } from 'events';
...
""")
            ...
            # 複雑なPythonファイル
            (project_root / "src" / "analytics_engine.py").write_text("""#!/usr/bin/env python3
\"\"\"
...
""")
    ...
    async def test_03_query_code_java_methods(self, tools, temp_project):
        ...
        # メソッド名の確認
        method_names = [r["content"] for r in result["results"] if "initialize" in r["content"]]
    ...
    async def test_04_query_code_typescript_interfaces(self, tools, temp_project):
        ...
        if "interface" in result["captures"]:
            ...
            assert any("DataItem" in name or "DataManagerConfig" in name for name in interface_names)
        
        print(f"✓ TypeScriptインターフェースクエリテスト成功: {result['total_count']}個検出")
    ...
    async def test_06_query_code_custom_query(self, tools, temp_project):
        ...
        # コンストラクタが見つかることを確認
        if result["count"] > 0:
            constructor_found = any("constructor" in r["content"].lower() for r in result["results"])
            assert constructor_found or result["count"] > 0  # Either explicit constructor or results found
    ...
    async def test_08_extract_and_query_workflow(self, tools, temp_project):
        ...
        print(f"✓ 抽出→クエリワークフローテスト成功: 抽出{extract_result['lines_extracted']}行 → クエリ{query_result['total_count']}個")
    ...
    async def test_09_large_file_performance(self, tools, temp_project):
        ...
        print(f"✓ 大規模ファイルパフォーマンステスト成功: {execution_time:.2f}秒で{result['count']}個の関数を解析")
    ...
    async def test_10_multi_language_consistency(self, tools, temp_project):
        ...
        test_cases = [
            ...
            {"file": "src/DataManager.ts", "language": "typescript", "query": "interfaces"},
            {"file": "src/analytics_engine.py", "language": "python", "query": "classes"}
        ]
    ...
    async def test_11_file_output_optimization(self, tools, temp_project):
        ...
        print(f"✓ ファイル出力最適化テスト成功: {result['count']}個の結果を{output_file.name}に保存")
    ...
    async def test_17_memory_efficiency(self, tools, temp_project):
        ...
        print(f"✓ メモリ効率性テスト成功: メモリ増加{memory_increase / 1024 / 1024:.1f}MB")
