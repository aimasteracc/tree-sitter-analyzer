...
class TestFileOutputManagerFactory:
    ...
    def test_path_normalization(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            manager2 = FileOutputManagerFactory.get_instance(str(Path(temp_dir).resolve()))
            assert manager1 is manager2
    
    def test_clear_instance(self):
        """Test clearing specific instances."""
        with tempfile.TemporaryDirectory() as temp_dir:
            manager = FileOutputManagerFactory.get_instance(temp_dir)
    ...
    def test_update_project_root(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir1:
            with tempfile.TemporaryDirectory() as temp_dir2:
                ...
                updated = FileOutputManagerFactory.update_project_root(temp_dir1, temp_dir2)
    ...
    def test_concurrent_access_different_project_roots(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir1:
            with tempfile.TemporaryDirectory() as temp_dir2:
                ...
                root1_instances = [inst for root, inst in instances if root == temp_dir1]
                root2_instances = [inst for root, inst in instances if root == temp_dir2]
