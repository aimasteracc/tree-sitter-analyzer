...
class TestRetiredResponseFormatsFailClosed:
    """JSON-only 迁移后，旧格式必须在进入缓存和业务工具前失败关闭。"""

    def test_toon_is_rejected_with_machine_repairable_error(self, edit_facade) -> None:
        base = {"action": "safe", "file_path": _TARGET}
        result = asyncio.run(edit_facade.execute({**base, "output_format": "toon"}))

        assert result["success"] is False
        assert result["error_code"] == "INVALID_ARGUMENT"
        assert result["invalid_arguments"] == ["output_format"]
        assert result["invalid_values"] == {"output_format": "toon"}
