...
class TestMaxCountFunctionality:
    ...
    async def test_max_count_applied_to_ripgrep(
        ...
        with patch(
            "tree_sitter_analyzer.mcp.tools.fd_rg_utils.build_rg_command"
        ) as mock_build_cmd:
            mock_build_cmd.return_value = ["rg", "--json", "test"]

            await tool.execute({"query": "test", "roots": [TEST_ROOT], "max_count": 5})
    ...
    async def test_max_count_default_value(self, mock_validate_roots, mock_run_command):
        ...
        with patch(
            ...
            await tool.execute({"query": "test", "roots": [TEST_ROOT]})
...
class TestCacheKeyGeneration:
    ...
    def test_different_output_formats_different_keys(self):
        ...
        key1 = cache.create_cache_key(
            query=args_total["query"], roots=args_total["roots"], total_only=True
        )
    ...
    def test_cache_key_includes_output_format_params(self):
        ...
        key_without_format = cache.create_cache_key(query="test", roots=[TEST_ROOT])
    ...
    def test_invalid_max_count_type(self):
        ...
        with pytest.raises(ValueError) as exc_info:
            tool.validate_arguments(
                {"query": "test", "roots": [TEST_ROOT], "max_count": "invalid"}
            )
...
class TestBackwardCompatibility:
    ...
    async def test_legacy_parameter_combinations(
        self, mock_validate_roots, mock_run_command
    ):
...
class TestMultilingualErrorMessages:
    ...
    def test_error_message_includes_efficiency_guide(self):
        ...
        assert (
            len(found_estimates) >= 3
        ), f"Expected at least 3 token estimates, found: {found_estimates}"
    ...
    def test_error_message_visual_formatting(self):
        ...
        assert (
            len(found_markers) >= 3
        ), f"Expected at least 3 visual markers, found: {found_markers}"
    ...
    def test_efficiency_guide_completeness(self):
        ...
        assert (
            expected_formats == guide_formats
        ...
        for format_name, description in validator.FORMAT_EFFICIENCY_GUIDE.items():
            ...
            assert (
                len(description) > 10
            ), f"Too short description for {format_name}: {description}"
    ...
    def test_backwards_compatibility_error_detection(self):
        ...
        assert (
            "排他的" in error_message
            or "exclusive" in error_message.lower()
            or "Multiple formats" in error_message
        )
