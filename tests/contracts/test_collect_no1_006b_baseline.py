...
@pytest.mark.parametrize(("path","value"),[(('source','source_archive_sha256'),'0'*64),(('dependency_closure','export_sha256'),'1'*64)])
def test_schema_rejects_frozen_subject_digest_mutation(path: tuple[str,...], value: str) -> None:
    with pytest.raises(ValidationError): collector.validate_receipt(mutated(path,value),schema())


@pytest.mark.parametrize(("section","field","value"),[("source","source_archive_sha256","0"*64),("dependency_closure","export_sha256","1"*64)])
def test_custom_validator_rejects_frozen_digest_with_weakened_schema(section: str, field: str, value: str) -> None:
    weakened=copy.deepcopy(schema()); weakened["properties"][section]["properties"][field]={"type":"string"}
...
def test_historical_detached_subject_archive_matches_frozen_digest(tmp_path: Path) -> None:
    ...
    assert collector.sha256(archive) == collector.EXPECTED_SUBJECT_ARCHIVE_SHA256 == baseline()["source"]["source_archive_sha256"]


def pinned_mac_uv() -> Path | None:
    discovered=shutil.which("uv")
    if sys.platform != "darwin" or discovered is None: return None
...
def test_historical_subject_export_matches_frozen_digest(tmp_path: Path) -> None:
    worktree=tmp_path/"subject"
    subprocess.run(["git","worktree","add","-q","--detach",str(worktree),collector.EXPECTED_SUBJECT_COMMIT],cwd=REPO,check=True)
    try:
        command=[str(pinned_mac_uv()),"export","--no-config","--frozen","--offline","--no-dev","--no-emit-project","--format","requirements-txt"]
        exported=subprocess.run(command,cwd=worktree,env=collector.clean_env(),check=True,capture_output=True).stdout
    finally: subprocess.run(["git","worktree","remove","--force",str(worktree)],cwd=REPO,check=True)
    assert collector.digest_bytes(exported) == collector.EXPECTED_SUBJECT_EXPORT_SHA256 == baseline()["dependency_closure"]["export_sha256"]
...
def test_receipt_binds_exact_collector_and_schema_bytes() -> None:
    ...
    paths=("scripts/collect_no1_006b_baseline.py","schemas/no1-006b-baseline.schema.json","scripts/no1_006b_baseline_support.py")
...
@pytest.mark.parametrize(("field","value"),[("machine","x86_64"),("soabi","cpython-314d-darwin"),("py_debug",1),("config_args_sha256","0"*64),("resolved_base_executable_sha256","1"*64)])
def test_no1_python_rejects_different_native_build_before_sampling(field: str, value: object) -> None:
    host={name:baseline()["environment"]["build_python"][name] for name in collector.NATIVE_IDENTITY_FIELDS}
    target={**host,field:value}
...
def test_finalize_receipt_uses_bound_schema_blob_without_live_read(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    observed=[]; bound=json.dumps(schema()).encode()
    ...
    monkeypatch.setattr(Path,"read_bytes",lambda self: pytest.fail("live schema read"))
    collector.finalize_receipt(baseline(),tmp_path/"receipt.json",REPO,bound)
...
def test_external_interpreter_probe_preserves_clean_ignored_gate(tmp_path: Path) -> None:
    ...
    shutil.copy2(collector.SUPPORT,root/"scripts/no1_006b_baseline_support.py")
    ...
    probe='import json; from scripts import collect_no1_006b_baseline as m; identity,schema=m.collector_identity("a"*64); print(json.dumps([identity,schema.decode()],sort_keys=True))'
