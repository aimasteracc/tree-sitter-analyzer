...
""")
    ...
    @pytest.mark.asyncio
    async def test_basic_file_output_text_format(self, read_partial_tool, temp_project_dir):
    ...
    @pytest.mark.asyncio
    async def test_suppress_output_functionality(self, read_partial_tool, temp_project_dir):
    ...
    @pytest.mark.asyncio
    async def test_suppress_output_without_file(self, read_partial_tool, temp_project_dir):
    ...
    async def test_java_file_extraction(self, read_partial_tool, temp_project_dir):
        ...
        assert "public class Sample" in json_content["content"] or "private String name" in json_content["content"]

    @pytest.mark.asyncio
    async def test_error_handling_file_save_failure(self, read_partial_tool, temp_project_dir):
        """Test error handling when file save fails"""
        test_file = Path(temp_project_dir) / "sample.py"
        
        # Mock FileOutputManager to raise an exception
        from unittest.mock import patch
        with patch.object(read_partial_tool.file_output_manager, 'save_to_file') as mock_save:
    ...
    def test_tool_definition_includes_new_parameters(self, read_partial_tool):
        ...
        assert "suppress partial_content_result" in properties["suppress_output"]["description"]
    ...
    @pytest.mark.asyncio
    async def test_automatic_base_name_generation(self, read_partial_tool, temp_project_dir):
    ...
    @pytest.mark.asyncio
    async def test_format_parameter_validation(self, read_partial_tool, temp_project_dir):
