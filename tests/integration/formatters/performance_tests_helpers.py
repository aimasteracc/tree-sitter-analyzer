...
class PerformanceTester:
    ...
    def generate_performance_report(self) -> str:
        ...
        report_file = (
            self.results_dir
            / f"performance_report_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}.md"
        )
    ...
    def _save_performance_results(
        ...
        results_file = (
            self.results_dir
            / f"{test_name}_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}.json"
        )
    ...
    def _save_scalability_results(self, result: ScalabilityTestResult):
        ...
        results_file = (
            self.results_dir
            / f"scalability_{result.test_name}_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}.json"
        )
