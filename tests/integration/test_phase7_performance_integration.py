...
'''
    ...
    async def test_large_scale_file_analysis_performance(self, large_scale_project):
        ...
        # パフォーマンス要件検証
        assert metrics["execution_time"] < 15.0, f"実行時間が15秒を超過: {metrics['execution_time']:.2f}秒"
        assert metrics["memory_mb"] < 200, f"メモリ使用量が200MBを超過: {metrics['memory_mb']:.2f}MB"
        
        # 結果検証
        successful_analyses = [r for r in results if r["success"]]
        assert len(successful_analyses) >= len(test_files) * 0.8, "80%以上の分析が成功する必要があります"
        
        print(f"大規模分析完了: {len(successful_analyses)}/{len(test_files)} 成功")
        print(f"実行時間: {metrics['execution_time']:.2f}秒, メモリ: {metrics['memory_mb']:.2f}MB")
    ...
    async def test_concurrent_search_performance(self, large_scale_project):
        ...
        # 結果検証
        successful_searches = [r for r in results if isinstance(r, dict) and r.get("success")]
        assert len(successful_searches) >= len(search_queries) * 0.8, "80%以上の検索が成功する必要があります"
        ...
        print(f"総マッチ数: {total_matches}, 実行時間: {metrics['execution_time']:.2f}秒")
    ...
    async def test_memory_efficiency_under_load(self, large_scale_project):
        ...
        # 20個のファイルを順次処理
        java_files = list(Path(large_scale_project).glob("src/main/java/**/*.java"))[:20]
        ...
        # 平均実行時間が合理的であることを確認
        avg_execution_time = sum(m["execution_time"] for m in memory_measurements) / len(memory_measurements)
        assert avg_execution_time < 2.0, f"平均実行時間が2秒を超過: {avg_execution_time:.2f}秒"
    ...
    async def test_scalability_limits(self, large_scale_project):
        ...
        for load_level in load_levels:
            ...
            for i in range(load_level):
                ...
                if isinstance(tool, AnalyzeScaleTool):
                    ...
                    # 異なるファイルの構造分析
                    python_files = list(Path(large_scale_project).glob("python/**/*.py"))
                    if python_files:
            ...
            print(f"負荷レベル {load_level}: {metrics['execution_time']:.2f}秒, "
                  f"成功率: {success_rate:.2%}, メモリ: {metrics['memory_mb']:.2f}MB")
            
            # 基本的な要件確認
            assert success_rate >= 0.7, f"負荷レベル {load_level} で成功率が70%を下回りました: {success_rate:.2%}"
        ...
        assert max_load_result["execution_time"] < 60.0, "最大負荷での実行時間が60秒を超過"
        assert max_load_result["memory_mb"] < 500, "最大負荷でのメモリ使用量が500MBを超過"
        
        print("スケーラビリティテスト完了:")
        for result in scalability_results:
            print(f"  負荷 {result['load_level']}: {result['execution_time']:.2f}秒, "
    ...
    async def test_sustained_load_performance(self, large_scale_project):
        ...
        success_rate = len(successful_iterations) / total_iterations if total_iterations > 0 else 0
        
        avg_execution_time = sum(s["execution_time"] for s in successful_iterations) / len(successful_iterations) if successful_iterations else 0
        avg_memory = sum(s["memory_mb"] for s in successful_iterations) / len(successful_iterations) if successful_iterations else 0
        
        # 持続負荷要件検証
        assert success_rate >= 0.95, f"持続負荷での成功率が95%を下回りました: {success_rate:.2%}"
        assert avg_execution_time < 3.0, f"平均実行時間が3秒を超過: {avg_execution_time:.2f}秒"
    ...
    async def test_resource_cleanup_efficiency(self, large_scale_project):
        ...
        for cycle in range(5):
            ...
            tools_and_params = [
                (AnalyzeScaleTool(large_scale_project), {
                    "file_path": str(list(Path(large_scale_project).glob("**/*.java"))[0]),
                    "include_complexity": True
                }),
                (TableFormatTool(large_scale_project), {
                    "file_path": str(list(Path(large_scale_project).glob("**/*.py"))[0]),
                    "format_type": "full"
                }),
                (SearchContentTool(large_scale_project), {
                    "roots": [large_scale_project],
            ...
            # メモリ増加が制御されていることを確認
            assert memory_growth < 200, f"サイクル {cycle} でメモリ増加が200MBを超過: {memory_growth:.2f}MB"
