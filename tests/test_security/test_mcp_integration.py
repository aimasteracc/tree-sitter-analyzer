...
class TestSecurityMCPIntegration:
    ...
    def test_security_validator_with_mcp_project_path(self):
        ...
        is_valid, error = validator.validate_file_path("../../../etc/passwd", self.project_root)
        assert not is_valid
    ...
    def test_security_error_handling_in_mcp_context(self):
        ...
        security_violations = [
            ("path_traversal", lambda: validator.validate_file_path("../../../etc/passwd")),
            ("dangerous_regex", lambda: validator.validate_regex_pattern(r"(.+)+")),
            ("input_too_long", lambda: validator.sanitize_input("a" * 2000, max_length=100)),
        ]
        ...
        for violation_type, violation_func in security_violations:
            ...
            if violation_type == "input_too_long":
                ...
                assert not is_valid, f"Security violation should be detected: {violation_type}"
                assert error != "", f"Error message should be provided for: {violation_type}"
    ...
    def test_security_logging_integration(self):
        ...
        security_operations = [
            ("valid_access", lambda: validator.validate_file_path("src/main.py", self.project_root)),
            ("blocked_access", lambda: validator.validate_file_path("../../../etc/passwd", self.project_root)),
        ...
        for operation_name, operation_func in security_operations:
            ...
            try:
                ...
                pytest.fail(f"Security operation {operation_name} raised unexpected exception: {e}")
    ...
    def test_performance_impact_on_mcp_server(self):
        ...
        operations = [
            ("file_validation", lambda: validator.validate_file_path("src/main.py", self.project_root)),
            ("regex_validation", lambda: validator.validate_regex_pattern(r"def\s+\w+")),
        ...
        for operation_name, operation_func in operations:
            ...
            assert avg_time < 0.01, f"{operation_name} too slow: {avg_time:.4f}s average"
