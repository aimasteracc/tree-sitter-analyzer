...
class TestFileOutputManagerDuplicationPrevention:
        ...
        with tempfile.TemporaryDirectory() as temp_dir1:
            with tempfile.TemporaryDirectory() as temp_dir2:
                ...
                with patch.dict('os.environ', {}, clear=True):
                    ...
                    fom1 = FileOutputManager()
                    
                    # プロジェクトルートを設定した2つ目のインスタンス
                    fom2 = FileOutputManager(project_root=temp_dir1)
                    
                    # 環境変数を設定した3つ目のインスタンス
                    with patch.dict('os.environ', {'TREE_SITTER_OUTPUT_PATH': temp_dir2}):
                        fom3 = FileOutputManager()
    ...
    def test_warning_suppression_across_modules(self):
        ...
        with patch.dict('os.environ', {}, clear=True):
            ...
            from tree_sitter_analyzer.mcp.utils.file_output_manager import FileOutputManager as FOM1
            ...
            if module_name in sys.modules:
                ...
                FOM2 = getattr(module, 'FileOutputManager')
            else:
    ...
    def test_no_warning_with_valid_environment(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.dict('os.environ', {'TREE_SITTER_OUTPUT_PATH': temp_dir}):
                ...
                assert len(FileOutputManager._warning_messages_shown) == initial_warnings
    ...
    def test_lock_file_expiration(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            try:
                ...
                # 期限切れなので警告表示が許可される
                assert FileOutputManager._should_show_warning(warning_key, max_age_seconds=300)
    ...
    def test_lock_file_path_generation(self):
        ...
        for warning_key, expected_filename in test_cases:
            lock_file = FileOutputManager._get_warning_lock_file(warning_key)
            expected_path = Path(tempfile.gettempdir()) / f"tree_sitter_analyzer_warning_{expected_filename}.lock"
