...
from tree_sitter_analyzer.languages.kotlin_plugin import KotlinPlugin, KotlinElementExtractor
...
class TestKotlinExtractorMissingLines:
    ...
    def test_extract_package_with_identifier_type(self):
        ...
        mock_grandchild.type = "qualified_identifier"  # Contains 'identifier' -> triggers line 190-192
    ...
    def test_extract_function_name_from_children_fallback(self):
        ...
        extractor._get_node_text = Mock(side_effect=lambda n: "testFunc" if n.type == "simple_identifier" else "")
    ...
    def test_extract_function_parameters_extraction(self):
        ...
        mock_node = Mock()
        mock_node.child_by_field_name = Mock(side_effect=lambda f: mock_params_node if f == "parameters" else (mock_name if f == "name" else None))
        mock_node.children = []
        mock_node.start_point = (0, 0)
    ...
    def test_extract_function_return_type_with_colon(self):
        ...
        mock_node = Mock()
        mock_node.child_by_field_name = Mock(side_effect=lambda f: mock_name if f == "name" else None)
    ...
    def test_extract_class_name_from_children_fallback(self):
        ...
        extractor._get_node_text = Mock(side_effect=lambda n: "TestClass" if n.type == "simple_identifier" else "class TestClass")
    ...
    def test_extract_class_visibility_from_modifiers(self):
        ...
        mock_node = Mock()
        mock_node.child_by_field_name = Mock(side_effect=lambda f: mock_name if f == "name" else mock_modifiers if f == "modifiers" else None)
        mock_node.children = []
        mock_node.start_point = (0, 0)
        mock_node.end_point = (0, 20)
        mock_node.start_byte = 0
        ...
        extractor._get_node_text = Mock(side_effect=lambda n: "Secret" if n == mock_name else "private" if n == mock_modifiers else "private class Secret")
    ...
    def test_extract_class_protected_visibility(self):
        ...
        extractor._get_node_text = Mock(side_effect=lambda n: "Protected" if n == mock_name else "protected" if n == mock_modifiers else "protected class Protected")
    ...
    def test_extract_class_internal_visibility(self):
        ...
        extractor._get_node_text = Mock(side_effect=lambda n: "Internal" if n == mock_name else "internal" if n == mock_modifiers else "internal class Internal")
...
class TestKotlinPluginMissingLines:
    ...
    def test_extract_elements_exception_handling(self):
        ...
        with patch.object(plugin, 'create_extractor', side_effect=Exception("Test error")):
