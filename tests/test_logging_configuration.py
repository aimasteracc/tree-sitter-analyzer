...
import os
utils_path = os.path.join(os.path.dirname(__file__), '..', 'tree_sitter_analyzer', 'utils.py')
...
class TestLoggingConfiguration(unittest.TestCase):
    ...
    def test_enable_file_logging_true(self):
        ...
        stream_handlers = [h for h in logger.handlers if h.__class__.__name__ == "SafeStreamHandler"]
    ...
    def test_file_logging_error_handling(self):
        ...
        os.environ["TREE_SITTER_ANALYZER_LOG_DIR"] = "\\\\invalid\\network\\path\\that\\does\\not\\exist"
    ...
    def test_file_log_level_string_conversion(self):
        ...
        for level_str, expected_level in test_cases:
            ...
            logger = setup_logger(f"test_logging_file_level_{level_str}", level=logging.WARNING)
