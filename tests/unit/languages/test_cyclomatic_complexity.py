"""Tests for cyclomatic complexity in Ruby, Kotlin, PHP, and Swift plugins.
...
def _swift_functions(source: str):
    ...
    parser = tree_sitter.Parser(lang)
    tree = parser.parse(source.encode())
    from tree_sitter_analyzer.languages._swift_plugin_elements import (
...
class TestSwiftCyclomaticComplexity:
    ...
    def test_binary_search(self):
        """while + if + else-if (nested if) = 3 decisions → complexity 4."""
