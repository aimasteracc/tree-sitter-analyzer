...
1.  builds & routes — factory returns FacadeTool, all 16 actions present.
...
11. end-to-end no strict leak — route overview + smart + index_status through
    REAL inners (tmp_path root) and assert no ValueError mentioning 'action'.
"""
...
# ---------------------------------------------------------------------------
# Expected action set (spec §0: 16 actions)
# ---------------------------------------------------------------------------
...
def test_all_16_actions_registered() -> None:
    facade = build_project_facade(project_root=None)
...
def test_all_actions_in_action_map_not_bespoke() -> None:
    ...
    assert len(facade.action_map) == 16
...
@pytest.mark.parametrize(
    ...
    ids=["overview", "smart", "index_status"],
)
...
def test_schema_includes_action_and_union_params() -> None:
    ...
    # action enum must contain all 16 actions
