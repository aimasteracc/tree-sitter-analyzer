...
from tree_sitter_analyzer.mcp.tools.fd_rg.result_parser import MAX_RESULTS_HARD_CAP
...
async def test_rg_45_large_output_truncation(monkeypatch, tmp_path):
    ...
    out = b"".join(jline(i) for i in range(MAX_RESULTS_HARD_CAP + 5))
    ...
    assert res["count"] == MAX_RESULTS_HARD_CAP
