...
# --- Consolidated fd command builder tests ---
# Replaces 94 individual test_fd_* functions with parameterized tests.


@pytest.mark.unit
class TestBuildFdCommandParameterized:
    """Parameterized tests for build_fd_command flag mapping."""

    @pytest.mark.parametrize(
        "kwargs,expected_flags",
        [
            # Basic pattern
            ({"pattern": "*.py", "glob": True, "roots": ["/test"]}, ["--glob", "*.py"]),
            # No pattern → default "."
            ({"pattern": None, "glob": False, "roots": ["/test"]}, ["."]),
            # Hidden files
            ({"hidden": True, "roots": ["/test"]}, ["-H"]),
            # No ignore
            ({"no_ignore": True, "roots": ["/test"]}, ["-I"]),
            # Follow symlinks
            ({"follow_symlinks": True, "roots": ["/test"]}, ["-L"]),
            # Full path match
            ({"full_path_match": True, "roots": ["/test"]}, ["-p"]),
            # Absolute paths
            ({"absolute": True, "roots": ["/test"]}, ["-a"]),
            # Depth limit
            ({"depth": 3, "roots": ["/test"]}, ["-d", "3"]),
            # Limit results
            ({"limit": 100, "roots": ["/test"]}, ["--max-results", "100"]),
            # File type
            ({"types": ["f"], "roots": ["/test"]}, ["-t", "f"]),
            # Extension
            ({"extensions": ["py", "js"], "roots": ["/test"]}, ["-e", "py", "-e", "js"]),
            # Extension with dot prefix stripped
            ({"extensions": [".py"], "roots": ["/test"]}, ["-e", "py"]),
            # Exclude
            ({"exclude": ["node_modules", "__pycache__"], "roots": ["/test"]}, ["-E", "node_modules", "-E", "__pycache__"]),
            # Size filter
            ({"size": ["+1M", "-100k"], "roots": ["/test"]}, ["-S", "+1M", "-S", "-100k"]),
            # Changed within
            ({"changed_within": "1day", "roots": ["/test"]}, ["--changed-within", "1day"]),
            # Changed before
            ({"changed_before": "2023-01-01", "roots": ["/test"]}, ["--changed-before", "2023-01-01"]),
            # Multiple roots
            ({"roots": ["/a", "/b"]}, ["/a", "/b"]),
            # Combined flags
            (
                {"pattern": "*.py", "glob": True, "hidden": True, "follow_symlinks": True, "depth": 5, "roots": ["/src"]},
                ["--glob", "-H", "-L", "-d", "5", "*.py"],
            ),
        ],
        ids=[
            "glob_pattern", "no_pattern_default", "hidden", "no_ignore",
            "follow_symlinks", "full_path_match", "absolute", "depth",
            "limit", "type_file", "extensions", "ext_dot_prefix",
            "exclude", "size", "changed_within", "changed_before",
            "multiple_roots", "combined_flags",
        ],
    )
    def test_build_fd_command_flags(self, kwargs, expected_flags):
        from tree_sitter_analyzer.mcp.tools.fd_rg_utils import build_fd_command
        ...
        cmd = build_fd_command(**defaults)

        assert cmd[0] == "fd"
        assert "--color" in cmd
        assert "never" in cmd
        for flag in expected_flags:
            assert flag in cmd, f"Expected {flag!r} in {cmd}"

    def test_build_fd_command_no_false_flags(self):
        from tree_sitter_analyzer.mcp.tools.fd_rg_utils import build_fd_command

        cmd = build_fd_command(
            pattern=None, glob=False, types=None, extensions=None,
            exclude=None, depth=None, follow_symlinks=False, hidden=False,
            no_ignore=False, size=None, changed_within=None,
            changed_before=None, full_path_match=False, absolute=False,
            limit=None, roots=["/test"],
        )
        # Minimal command: fd --color never . /test
        assert cmd == ["fd", "--color", "never", ".", "/test"]


@pytest.mark.unit
class TestBuildRgCommandParameterized:
    """Parameterized tests for build_rg_command flag mapping."""
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ({"word": True}, ["-w"]),
            ({"max_count": 50}, ["-m", "50"]),
            ({"count_only_matches": True}, ["--count-matches"]),
            ({"include_globs": ["*.py"]}, ["-g", "*.py"]),
            ({"exclude_globs": ["*_test.py"]}, ["-g", "!*_test.py"]),
            ({"follow_symlinks": True}, ["-L"]),
            ({"hidden": True}, ["-H"]),
            ({"no_ignore": True}, ["-u"]),
        ],
        ids=[
            "case_insensitive", "case_smart", "case_sensitive",
            "fixed_strings", "multiline", "word_match",
            "max_count", "count_matches", "include_glob",
            "exclude_glob", "follow_symlinks", "hidden", "no_ignore",
        ],
    )
    def test_build_rg_command_flags(self, kwargs, expected_flags):
        from tree_sitter_analyzer.mcp.tools.fd_rg_utils import build_rg_command
        ...
        defaults = {
            ...
            "timeout_ms": None, "roots": ["/test"], "files_from": None,
            "count_only_matches": False,
        }
        defaults.update(kwargs)
        cmd = build_rg_command(**defaults)
...
@pytest.mark.unit
@pytest.mark.asyncio
class TestFdExecuteIntegration:
    """Integration tests for ListFilesTool.execute() with mocked subprocess."""

    async def test_list_files_basic_search(self, monkeypatch, tmp_path):
        (tmp_path / "a.py").write_text("print('hi')\n", encoding="utf-8")
        tool = ListFilesTool(str(tmp_path))

        async def fake_run(cmd, input_data=None, timeout_ms=None):
            return 0, f"{tmp_path / 'a.py'}\n".encode(), b""
    ...
    async def test_list_files_count_only(self, monkeypatch, tmp_path):
        tool = ListFilesTool(str(tmp_path))

        async def fake_run(cmd, input_data=None, timeout_ms=None):
            files = [str(tmp_path / f"f{i}.py") for i in range(5)]
            return 0, "\n".join(files).encode(), b""
    ...
    async def test_list_files_error_handling(self, monkeypatch, tmp_path):
        tool = ListFilesTool(str(tmp_path))

        async def fake_run(cmd, input_data=None, timeout_ms=None):
            return 1, b"", b"error: fd not found"
    ...
    async def test_list_files_with_size_and_time_filters(self, monkeypatch, tmp_path):
        (tmp_path / "big.py").write_text("x" * 1000, encoding="utf-8")
        tool = ListFilesTool(str(tmp_path))

        async def fake_run(cmd, input_data=None, timeout_ms=None):
            assert "-S" in cmd
            return 0, f"{tmp_path / 'big.py'}\n".encode(), b""

        monkeypatch.setattr(
            "tree_sitter_analyzer.mcp.tools.fd_rg_utils.run_command_capture", fake_run
        )
        result = await tool.execute(
            {"roots": [str(tmp_path)], "size": ["+100c"], "output_format": "json"}
        )
        assert result["success"] is True

    async def test_list_files_with_depth_and_symlinks(self, monkeypatch, tmp_path):
        tool = ListFilesTool(str(tmp_path))
        ...
        await tool.execute(
            {"roots": [str(tmp_path)], "depth": 3, "follow_symlinks": True}
        )

    async def test_list_files_exclude_pattern(self, monkeypatch, tmp_path):
        f1 = tmp_path / "keep.py"
        f1.write_text("pass\n", encoding="utf-8")
        tool = ListFilesTool(str(tmp_path))
        ...
        result = await tool.execute(
            {"roots": [str(tmp_path)], "exclude": ["*.tmp"], "output_format": "json"}
        )
        assert result["success"] is True
