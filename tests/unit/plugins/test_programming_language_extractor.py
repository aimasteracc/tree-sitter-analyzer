#!/usr/bin/env python3
"""
Unit tests for ProgrammingLanguageExtractor.

Tests cover:
- Initialization and cache management
- AST traversal (iterative with depth limits)
- Element caching and processed node tracking
...
class TestProgrammingLanguageExtractorInit:
    """Tests for ProgrammingLanguageExtractor initialization."""

    def test_init_creates_programming_specific_caches(self):
        """Test that initialization creates programming-specific caches."""
        from tree_sitter_analyzer.plugins.programming_language_extractor import (
            ProgrammingLanguageExtractor,
        )

        class TestExtractor(ProgrammingLanguageExtractor):
        ...
        # Check programming-specific caches
    ...
    def test_init_inherits_from_cached_element_extractor(self):
        """Test that ProgrammingLanguageExtractor inherits from CachedElementExtractor."""
        ...
        assert issubclass(ProgrammingLanguageExtractor, CachedElementExtractor)
...
class TestCacheManagement:
    ...
    def test_reset_caches_clears_all_caches(self, extractor):
        """Test that _reset_caches clears all caches including programming-specific ones."""
        # Populate caches
...
class TestContainerNodeTypes:
    """Tests for container node type management."""
    ...
    def test_subclass_can_override_container_types(self):
        ...
        class JavaExtractor(ProgrammingLanguageExtractor):
...
class TestASTTraversal:
    ...
    def test_traverse_handles_extractor_exception(self, extractor):
...
class TestComplexityCalculation:
    ...
    def test_subclass_can_override_decision_keywords(self):
        """Test that subclass can extend decision keywords."""
        from tree_sitter_analyzer.plugins.programming_language_extractor import (
            ProgrammingLanguageExtractor,
        )

        class CustomExtractor(ProgrammingLanguageExtractor):
            def _get_decision_keywords(self):
                return super()._get_decision_keywords() | {
                    "switch_statement",
                    "try_statement",
                }
        ...
        ext = CustomExtractor()
        keywords = ext._get_decision_keywords()

        assert "switch_statement" in keywords
        assert "try_statement" in keywords
        assert "if_statement" in keywords  # From parent


class TestObjectIDTracking:
    """Tests for object ID-based tracking (vs position-based)."""
    ...
    def test_processed_nodes_uses_object_id(self, extractor):
        """Test that processed nodes tracking uses object IDs."""
        mock_node = MagicMock()
        node_id = id(mock_node)

        extractor._processed_nodes.add(node_id)

        assert node_id in extractor._processed_nodes
        assert isinstance(node_id, int)

    def test_element_cache_uses_object_id_and_type(self, extractor):
        """Test that element cache uses (object_id, element_type) as key."""
        mock_node = MagicMock()
        node_id = id(mock_node)
        element_type = "function"

        cache_key = (node_id, element_type)
        extractor._element_cache[cache_key] = {"name": "test"}

        assert cache_key in extractor._element_cache
        assert isinstance(cache_key[0], int)
