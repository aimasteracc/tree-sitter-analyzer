...
def match_result(draw):
    ...
    return {
        ...
        "match": draw(st.text(min_size=1, max_size=50, alphabet=st.characters(
            blacklist_categories=('Cs',),
            blacklist_characters=('\x00', '\r', '\n')
        ))),
    }
...
def analysis_result_data(draw):
    ...
    return {
        ...
        "language": draw(st.sampled_from(["python", "java", "javascript", "typescript"])),
        "success": True,
        "line_count": draw(st.integers(min_value=1, max_value=10000)),
        "classes": draw(st.lists(
            st.fixed_dictionaries({
                "name": st.sampled_from(["MyClass", "TestClass", "Service", "Handler"]),
        ...
        "methods": draw(st.lists(
            st.fixed_dictionaries({
                "name": st.sampled_from(["main", "test", "process", "handle", "execute"]),
                "line": st.integers(min_value=1, max_value=1000),
            }),
            min_size=0,
            max_size=10,
...
class TestCLIOutputFormatProperties:
    ...
    **Validates: Requirements 9.2**
    ...
    def test_property_12_json_format_produces_valid_json(self, data: Any, capsys):
        ...
        **Validates: Requirements 9.2**
        ...
        try:
            ...
            pytest.fail(f"JSON format should produce valid JSON: {e}\nOutput was: {captured.out}")
    ...
    def test_property_12_json_format_roundtrip_preserves_data(self, data: Any, capsys):
        ...
        **Validates: Requirements 9.2**
        ...
        if isinstance(data, dict):
            assert isinstance(parsed, dict), "Dict data should remain dict after round-trip"
            ...
            assert isinstance(parsed, list), "List data should remain list after round-trip"
    ...
    def test_property_12_search_result_json_contains_required_fields(self, data: dict, capsys):
        """
        **Feature: test-coverage-improvement, Property 12: CLI Output Format Consistency**
        ...
        **Validates: Requirements 9.2**
        ...
        assert "total_matches" in parsed, "Search result should contain 'total_matches' field"
    ...
    def test_property_12_list_files_result_json_contains_required_fields(self, data: dict, capsys):
        """
        **Feature: test-coverage-improvement, Property 12: CLI Output Format Consistency**
    ...
    def test_property_12_analysis_result_json_contains_required_fields(self, data: dict, capsys):
        """
        **Feature: test-coverage-improvement, Property 12: CLI Output Format Consistency**
    ...
    def test_property_12_output_mode_configuration(self, quiet: bool, json_output: bool, capsys):
        """
        **Feature: test-coverage-improvement, Property 12: CLI Output Format Consistency**
    ...
    @PROPERTY_SETTINGS
    @given(items=st.lists(st.text(min_size=1, max_size=50, alphabet=st.characters(
        blacklist_categories=('Cs',),
        blacklist_characters=('\x00',)
        ...
        **Validates: Requirements 9.2**
        ...
        for item in items:
            assert item in captured.out, \
                f"Item '{item}' should appear in list output"
    ...
    **Validates: Requirements 9.2**
    ...
    def test_property_12_arbitrary_dict_json_format(self, data: dict, capsys):
        ...
        **Validates: Requirements 9.2**
        ...
        try:
            parsed = json.loads(captured.out)
            assert isinstance(parsed, dict), "Dict input should produce dict JSON output"
        except json.JSONDecodeError as e:
    ...
    def test_property_12_arbitrary_list_json_format(self, data: list, capsys):
        ...
        **Validates: Requirements 9.2**
        ...
        try:
            parsed = json.loads(captured.out)
            assert isinstance(parsed, list), "List input should produce list JSON output"
    ...
    def test_property_12_integer_count_json_format(self, count: int, capsys):
        ...
        **Validates: Requirements 9.2**
        ...
        assert parsed == count, f"Integer {count} should be preserved exactly, got {parsed}"

    @PROPERTY_SETTINGS
    @given(error_msg=st.text(min_size=1, max_size=200, alphabet=st.characters(
        blacklist_categories=('Cs',),
        blacklist_characters=('\x00',)
    )))
    def test_property_12_error_output_contains_message(self, error_msg: str, capsys):
        ...
        **Validates: Requirements 9.2**
        ...
        assert "ERROR:" in captured.err, \
            "Error output should be prefixed with 'ERROR:'"
