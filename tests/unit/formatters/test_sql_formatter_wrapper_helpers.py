...
def _make_element(name="test", element_type="table", start_line=1, end_line=10,
                  raw_text="CREATE TABLE test (id INT);", language="sql"):
...
class TestElementToDict:
    def test_full_attributes(self):
        el = SQLTable(name="users", start_line=1, end_line=5,
                      raw_text="CREATE TABLE users (id INT);", language="sql")
...
class TestConvertAnalysisResultToSqlElements:
    ...
    def test_sql_element_passthrough(self):
        table = SQLTable(name="existing", start_line=1, end_line=5,
                         raw_text="CREATE TABLE existing (id INT);", language="sql")
    ...
    def test_extractor_populates_columns(self):
        ...
        sqls = convert_analysis_result_to_sql_elements(
            result, extract_table_columns=extract_columns,
            extract_view_info=_noop_extractor, extract_procedure_info=_noop_extractor,
            extract_function_info=_noop_extractor, extract_trigger_info=_noop_extractor,
            extract_index_info=_noop_extractor,
        )
    ...
    def test_view_extractor_populates_source_tables(self):
        el = _make_element("v", element_type="view",
                           raw_text="CREATE VIEW v AS SELECT * FROM t1;")
        ...
        result = self._make_result([el])
        sqls = convert_analysis_result_to_sql_elements(
            result, extract_table_columns=_noop_extractor,
            extract_view_info=extract_view, extract_procedure_info=_noop_extractor,
            extract_function_info=_noop_extractor, extract_trigger_info=_noop_extractor,
            extract_index_info=_noop_extractor,
        )
        assert isinstance(sqls[0], SQLView)
    ...
    def test_procedure_extractor_populates_params(self):
        el = _make_element("sp", element_type="procedure",
        ...
        sqls = convert_analysis_result_to_sql_elements(
            result, extract_table_columns=_noop_extractor,
            extract_view_info=_noop_extractor, extract_procedure_info=extract_proc,
            extract_function_info=_noop_extractor, extract_trigger_info=_noop_extractor,
            extract_index_info=_noop_extractor,
        )
        assert isinstance(sqls[0], SQLProcedure)
    ...
    def test_function_extractor_populates_return_type(self):
        el = _make_element("fn", element_type="sql_function",
        ...
        sqls = convert_analysis_result_to_sql_elements(
            result, extract_table_columns=_noop_extractor,
            extract_view_info=_noop_extractor, extract_procedure_info=_noop_extractor,
            extract_function_info=extract_fn, extract_trigger_info=_noop_extractor,
            extract_index_info=_noop_extractor,
        )
        assert isinstance(sqls[0], SQLFunction)
    ...
    def test_trigger_extractor_populates_timing(self):
        ...
        sqls = convert_analysis_result_to_sql_elements(
            result, extract_table_columns=_noop_extractor,
            extract_view_info=_noop_extractor, extract_procedure_info=_noop_extractor,
            extract_function_info=_noop_extractor, extract_trigger_info=extract_trg,
            extract_index_info=_noop_extractor,
        )
        assert isinstance(sqls[0], SQLTrigger)
    ...
    def test_index_extractor_populates_unique(self):
        el = _make_element("idx", element_type="index",
                           raw_text="CREATE UNIQUE INDEX idx ON t (c)")
...
class TestCreateSqlElementFromDict:
    def test_table(self):
        result = create_sql_element_from_dict({
            "type": "table", "name": "t", "start_line": 1, "end_line": 5,
            "raw_text": "CREATE TABLE t (id INT);", "language": "sql",
        })
    ...
    def test_create_table_prefix(self):
        result = create_sql_element_from_dict({
            "type": "create_table", "name": "ct", "start_line": 1, "end_line": 5,
            "raw_text": "CREATE TABLE ct (id INT);", "language": "sql",
        })
        assert isinstance(result, SQLTable)
    ...
    def test_view(self):
        result = create_sql_element_from_dict({
            "type": "view", "name": "v", "start_line": 1, "end_line": 3,
            "raw_text": "CREATE VIEW v AS SELECT 1;", "language": "sql",
        })
        assert isinstance(result, SQLView)

    def test_create_view_prefix(self):
        result = create_sql_element_from_dict({
            "type": "create_view", "name": "cv", "start_line": 1, "end_line": 3,
            "raw_text": "CREATE VIEW cv AS SELECT 1;", "language": "sql",
        })
        assert isinstance(result, SQLView)

    def test_procedure(self):
        result = create_sql_element_from_dict({
            "type": "procedure", "name": "sp", "start_line": 1, "end_line": 10,
            "raw_text": "CREATE PROCEDURE sp() BEGIN END;", "language": "sql",
        })
        assert isinstance(result, SQLProcedure)

    def test_create_procedure_prefix(self):
        result = create_sql_element_from_dict({
            "type": "create_procedure", "name": "csp", "start_line": 1, "end_line": 10,
            "raw_text": "CREATE PROCEDURE csp() BEGIN END;", "language": "sql",
        })
        assert isinstance(result, SQLProcedure)

    def test_function(self):
        result = create_sql_element_from_dict({
            "type": "function", "name": "fn", "start_line": 1, "end_line": 8,
            "raw_text": "CREATE FUNCTION fn() RETURNS INT;", "language": "sql",
        })
        assert isinstance(result, SQLFunction)

    def test_create_function_prefix(self):
        result = create_sql_element_from_dict({
            "type": "create_function", "name": "cfn", "start_line": 1, "end_line": 8,
            "raw_text": "CREATE FUNCTION cfn() RETURNS INT;", "language": "sql",
        })
        assert isinstance(result, SQLFunction)

    def test_trigger(self):
        result = create_sql_element_from_dict({
            "type": "trigger", "name": "trg", "start_line": 1, "end_line": 6,
            "raw_text": "CREATE TRIGGER trg BEFORE UPDATE ON t;", "language": "sql",
        })
        assert isinstance(result, SQLTrigger)

    def test_create_trigger_prefix(self):
        result = create_sql_element_from_dict({
            "type": "create_trigger", "name": "ctrg", "start_line": 1, "end_line": 6,
            "raw_text": "CREATE TRIGGER ctrg BEFORE UPDATE ON t;", "language": "sql",
        })
        assert isinstance(result, SQLTrigger)

    def test_index(self):
        result = create_sql_element_from_dict({
            "type": "index", "name": "idx", "start_line": 1, "end_line": 1,
            "raw_text": "CREATE INDEX idx ON t (c);", "language": "sql",
        })
        assert isinstance(result, SQLIndex)

    def test_create_index_prefix(self):
        result = create_sql_element_from_dict({
            "type": "create_index", "name": "cidx", "start_line": 1, "end_line": 1,
            "raw_text": "CREATE INDEX cidx ON t (c);", "language": "sql",
        })
        assert isinstance(result, SQLIndex)

    def test_unknown_type_falls_back_to_table(self):
        result = create_sql_element_from_dict({
            "type": "unknown", "name": "x", "start_line": 1, "end_line": 1,
            "raw_text": "SOME SQL;", "language": "sql",
        })
        assert isinstance(result, SQLTable)
    ...
    def test_all_dict_factory_types_produce_correct_classes(self):
        ...
        for type_name, expected_cls in type_expected.items():
            result = create_sql_element_from_dict({
                "type": type_name, "name": f"test_{type_name}",
            })
