...
class TestMatchNodes:
    def test_match_by_name(self):
        ...
        old = [ASTNodeInfo(
            node_type="function_definition", kind=ASTNodeKind.FUNCTION,
            name="foo", start_line=1, start_col=0, end_line=5, end_col=0,
            text_hash="a", text_preview="def foo(): pass",
        )]
        new = [ASTNodeInfo(
            node_type="function_definition", kind=ASTNodeKind.FUNCTION,
            name="foo", start_line=1, start_col=0, end_line=5, end_col=0,
            text_hash="b", text_preview="def foo(): return 1",
        )]
    ...
    def test_no_match(self):
        ...
        new = [ASTNodeInfo(
            node_type="function_definition", kind=ASTNodeKind.FUNCTION,
            name="bar", start_line=1, start_col=0, end_line=5, end_col=0,
            text_hash="b", text_preview="def bar(): pass",
        )]
        matched, old_rem, new_rem = _match_nodes(old, new)
...
class TestDiffStrings:
    ...
    def test_added_function(self, differ):
        ...
        added_kinds = [h.node_kind for h in result.hunks if h.diff_kind == DiffKind.NODE_ADDED]
    ...
    def test_to_dict_roundtrip(self, differ):
        old = "def foo():\n    pass\n"
        new = "def foo():\n    return 1\n"
        result = differ.diff_strings(old, new, "python", old_file="a.py", new_file="a.py")
        d = result.to_dict()
        assert "hunks" in d
        assert "summary" in d
...
class TestExtractSignature:
    def test_basic(self):
        ...
        node = ASTNodeInfo(
            node_type="function_definition", kind=ASTNodeKind.FUNCTION,
            name="foo", start_line=1, start_col=0, end_line=3, end_col=0,
            text_hash="abc", text_preview="def foo(x): pass",
            children=[
                ASTNodeInfo(
                    node_type="parameters", kind=ASTNodeKind.PARAMETER,
                    name="", start_line=1, start_col=8, end_line=1, end_col=11,
                    text_hash="p1", text_preview="(x)",
                ),
            ],
        )
