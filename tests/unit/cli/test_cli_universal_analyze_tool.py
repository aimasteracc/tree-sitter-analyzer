#!/usr/bin/env python3
"""
Tests for UniversalAnalyzeTool

This test module provides coverage for the UniversalAnalyzeTool class,
...
class TestArgumentValidation:
    ...
    def test_validate_arguments_invalid_analysis_type_type(self):
        """Test validation fails with non-string analysis_type"""
        tool = UniversalAnalyzeTool()
        args = {"file_path": "test.py", "analysis_type": 123}
...
class TestEdgeCases:
    ...
    @pytest.mark.asyncio
    async def test_execute_with_explicit_language(self, tmp_path):
        """Test execution with explicitly specified language for non-standard extension."""
        test_file = tmp_path / "test.txt"
        test_file.write_text("def hello(): pass")

        tool = UniversalAnalyzeTool(str(tmp_path))
        args = {"file_path": str(test_file), "language": "python"}
...
class TestJavaFileAnalysis:
    """Test Java file analysis with real temp files."""

    @pytest.fixture
    def tool(self, tmp_path):
        """Create a UniversalAnalyzeTool instance for testing."""
        return UniversalAnalyzeTool(str(tmp_path))

    @pytest.fixture
    def sample_java_file(self, tmp_path):
        """Create a sample Java file for testing."""
        file_path = Path(tmp_path) / "Sample.java"
