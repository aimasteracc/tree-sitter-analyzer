...
class TestLLMGuidanceCompliance:
    ...
    def test_parameter_exclusive_markers(self):
        ...
        for param in exclusive_params:
            ...
            assert "RECOMMENDED for:" in param_desc, f"Missing recommendation in {param}"
            assert "Cannot be combined" in param_desc, f"Missing combination warning in {param}"
    ...
    def test_parameter_token_estimates(self):
        ...
        for param, estimate in token_estimates.items():
            param_desc = properties[param]["description"]
            assert estimate in param_desc, f"Missing token estimate '{estimate}' in {param}"
    ...
    def test_parameter_recommendations(self):
        ...
        for param, recommendation in recommendations.items():
            ...
            assert recommendation.lower() in param_desc.lower(), f"Missing recommendation '{recommendation}' in {param}"
    ...
    def test_description_length_reasonable(self):
        ...
        assert min_length <= len(description) <= max_length, (
    ...
    def test_unicode_markers_present(self):
        ...
        for marker in visual_markers:
            ...
            assert description.count(marker) >= 1, f"Insufficient usage of marker: {marker}"
