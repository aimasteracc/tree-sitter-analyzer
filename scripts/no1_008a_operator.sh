...
contract(){ printf '%s\n' '{"schema_version":1,"cells":14,"attempts_per_cell":1,"max_concurrency":1,"roles":["producer","authority-service","executor-service","approver-service","fresh-verifier"],"qualification":"production-root-exact-14-only"}'; }
...
declare CONTRACTS_DIR='' AUTHORITY_SOCKET='' EXECUTOR_SOCKET='' APPROVER_SOCKET='' PUBLIC_CONFIG='' ARTIFACT_ROOT='' STAGED_ROOT='' EXPERIMENT_ROOT=''
 ...
 *) usage;; esac; done
for value in "$CONTRACTS_DIR" "$AUTHORITY_SOCKET" "$EXECUTOR_SOCKET" "$APPROVER_SOCKET" "$PUBLIC_CONFIG" "$ARTIFACT_ROOT" "$STAGED_ROOT"; do
...
exec python3 -m benchmarks.codegraph_compare.qualification_operator \
 ...
 --public-config "$PUBLIC_CONFIG" --artifact-root "$ARTIFACT_ROOT" \
 --staged-root "$STAGED_ROOT" --experiment-root "$EXPERIMENT_ROOT"
