...
class ReadmeConfig:
    ...
    statistics = [
        StatisticPattern(
            name="test_count",
            patterns=[
                r'tests-\d+%20passed'  # Only match badge URLs, not text content
            ],
    ...
    stat_commands = {
        ...
        'coverage': ['uv', 'run', 'pytest', 'tests/', '--cov=tree_sitter_analyzer', '--cov-report=term-missing'],
        'bigservice_analysis': ['uv', 'run', 'python', '-m', 'tree_sitter_analyzer', 'examples/BigService.java', '--advanced', '--output-format=text']
    }
