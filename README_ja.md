...
> 事前インデックス AST キャッシュ + 59 MCP ツール + 13 のキュレーション済みエージェント スキル + TOON 圧縮出力。
...
各 skill は `allowed-tools` ツール サブセット + 手順レシピ + 決定面スキーマを同梱し、エージェントは 59 個のツールから毎回選別する必要が無い。
...
インデックスは最初のクエリで遅延構築され、ファイル変更時はコンテンツ ハッシュ差分で増分更新 (`codegraph_incremental_sync`)。59 ツール全てが同じ `.ast-cache/` を共有し、クエリとフォローアップは作業を共有する。
