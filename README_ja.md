...
[![テスト](https://img.shields.io/badge/tests-4770%20passed-brightgreen.svg)](#8--品質保証)
...
[![バージョン](https://img.shields.io/badge/version-1.9.16-blue.svg)](https://github.com/aimasteracc/tree-sitter-analyzer/releases)
...
| **YAML** | 🆕 完全サポート | マッピング、シーケンス、アンカー、エイリアス、マルチドキュメントサポート、コメント |
| **Go** | 🆕 完全サポート | パッケージ、関数、メソッド、構造体、インターフェース、ゴルーチン、チャンネル |
| **Rust** | 🆕 完全サポート | モジュール、関数、構造体、列挙型、トレイト、実装ブロック、マクロ |
| **Kotlin** | 🆕 完全サポート | クラス、オブジェクト、インターフェース、関数、プロパティ、拡張関数、コルーチン |

**注記:** 現在、上記15言語が完全なプラグイン実装を持っています。C/C++、JSONなどは`LanguageDetector`に定義されていますが、現時点では機能的なプラグイン実装がありません。

### 🏆 本番環境対応
- **4,770のテスト** - 100%合格率、エンタープライズグレードの品質保証
- **高カバレッジ** - 包括的なテストスイート
...
**完全なYAML言語サポート**が次のリリースで提供予定です：

- **構造抽出**：マッピング（キー・バリューペア）、シーケンス（リスト）、スカラー値
- **高度な機能**：
  - アンカー（&anchor）とエイリアス（*alias）の検出
  - マルチドキュメントサポート（---区切り）
  - コメント抽出
  - ネストレベル計算
- **スカラー型識別**：文字列、数値、ブール値、nullの自動識別
- **YAMLフォーマッター**：専用出力フォーマッター、サマリー、構造、高度な解析、テーブル形式をサポート
- **Tree-sitterクエリサポート**：複雑なYAMLパターン解析
- **完全統合**：CLI、API、MCPインターフェースで利用可能

DevOpsエンジニアやKubernetes、Docker Compose、AnsibleなどのYAML設定ファイルを扱う開発者に最適！
...
- **SQL単一行解析**：ビュー定義解析の問題を修正
- **SQLトリガー行番号抽出**：修正と冗長なゴールデンマスターファイルのクリーンアップ
- **SQLパラメータ抽出正規表現**：CI失敗を解決するための改善
- **クロスプラットフォーム互換性**：SQL解析の堅牢性向上
