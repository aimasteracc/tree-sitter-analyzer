...
> 事前インデックス AST キャッシュ + 61 MCP ツール + 13 のキュレーション済みエージェント スキル + TOON 圧縮出力。
> 6 リポジトリの実測比較で **CodeGraph を上回る**（コスト中央値 **−11% vs CodeGraph の −4%**）、CLI は厳密な上位互換。

[![PyPI](https://img.shields.io/pypi/v/tree-sitter-analyzer.svg)](https://pypi.org/project/tree-sitter-analyzer/)
[![Python Version](https://img.shields.io/badge/python-3.10%2B-blue.svg)](https://python.org)
[![License](https://img.shields.io/badge/license-MIT-green.svg)](LICENSE)
[![Tests](https://img.shields.io/badge/tests-18702%20passed-brightgreen.svg)](#-品質とテスト)
...
各 skill は `allowed-tools` ツール サブセット + 手順レシピ + 決定面スキーマを同梱し、エージェントは 61 個のツールから毎回選別する必要が無い。
...
インデックスは最初のクエリで遅延構築され、ファイル変更時はコンテンツ ハッシュ差分で増分更新 (`codegraph_incremental_sync`)。61 ツール全てが同じ `.ast-cache/` を共有し、クエリとフォローアップは作業を共有する。
