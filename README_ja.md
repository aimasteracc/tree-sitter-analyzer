...
**なぜ違うのか：**
* **クロスランゲージ束縛は言語ファミリで門控されます。** 名前の一致だけではクロスランゲージの辺は生まれません。これを強制するゲートは規約ではなく実行されるテストです。
...
ここに集めたのは、そのまま読むと宣伝文句に見えてしまう範囲の記述です。インストール手順を中断しないよう、この位置にまとめています。

### 応答サイズとパラメータ名

`nav action=navigate` は一致したすべての定義の本体を展開し、切り詰めを報告しません。複数のファイルのクラスが共有するシンボル名では、参照集合の全体が同じ応答で返ります。まず `search action=symbol` で絞り込むか、`limit` を尊重して `truncated` を設定する `nav action=callers` / `action=callees` を使ってください。

`symbol` と `function_name` は `callers`、`callees`、`impact` のいずれでも受け付けられます。`navigate` と `lineage` は `symbol` を要求し、`function_name` を渡すと verdict エンベロープではなく例外になります。
...
### Pulse / TQL / セマンティック クエリ

これらは `nav` のアクションと内部 API を支えるサブシステムであり、エージェントが設定するツール面の一部ではありません。制限は推測に委ねず、明示します。
