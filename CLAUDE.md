...
2. **Assert "is it good?", not only "does it match?".** Conformance tests (`assert field == X`, `set(keys) <= SURFACE`) verify intent; they cannot question intent. Add at least one test that measures the *value* (bytes, ratio, count) and bounds it.
