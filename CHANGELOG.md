...
#### Go, Rust, Kotlin言語サポート追加！🆕

**Go言語サポート**:
- **構造抽出**: パッケージ、関数、メソッド、構造体、インターフェース
- **高度な機能**:
  - ゴルーチンとチャンネルパターンの検出
  - タイプエイリアス、定数、変数の抽出
- **Goフォーマッター**: Go専用用語を使用した出力

**Rust言語サポート**:
- **構造抽出**: モジュール、関数、構造体、列挙型、トレイト、実装ブロック（impl）
- **高度な機能**:
  - マクロ定義の抽出
  - Async関数とライフタイムアノテーションの検出
- **Rustフォーマッター**: Rust専用用語を使用した出力

**Kotlin言語サポート**:
- **構造抽出**: クラス、データクラス、シールドクラス、オブジェクト、インターフェース
- **高度な機能**:
  - 関数、プロパティ、拡張関数の抽出
  - Suspend関数とコルーチンパターンの検出
- **Kotlinフォーマッター**: Kotlin専用用語を使用した出力

これらの言語はCLI、API、MCPインターフェースに完全統合されており、プロパティベーステストによる品質保証も行われています。

#### YAML言語サポート追加！🆕
- **完全なYAML言語サポート**: 包括的なYAML解析機能を追加
  ...
  - **高度な機能**:
    ...
    - コメント抽出
    - ネストレベル計算
  - **スカラー型識別**: 文字列、数値、ブール値、null の自動識別
  - **YAMLフォーマッター**: YAML出力専用フォーマッター
    - サマリー、構造、高度な解析、テーブル形式をサポート
    - text、json、csv出力形式に対応
  - **Tree-sitterクエリサポート**: 複雑なYAMLパターン解析
  - **CLI・API・MCPインターフェイスに完全統合**
  - **プロパティベーステスト**: 13のプロパティテストで品質保証

- **対応ファイル拡張子**: `.yaml`, `.yml`
- **依存関係**: `tree-sitter-yaml>=0.7.0` をオプショナル依存として追加

### 🧪 テスト・品質向上

- **YAMLゴールデンマスターテスト追加**: YAMLファイルのリグレッションテストを追加
  - `tests/test_yaml/test_yaml_golden_master.py` - YAML専用ゴールデンマスターテスト
  - `tests/golden_masters/full/yaml_sample_config_full.md` - YAMLゴールデンマスターファイル
