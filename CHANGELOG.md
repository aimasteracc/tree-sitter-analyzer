...
- **Semantic Code Search**: `search/` module for natural language and pattern-based code search with hybrid architecture. Features query classification (simple vs complex), fast path execution (grep/ripgrep/ast-grep), LLM integration (OpenAI/Anthropic support), adaptive learning (cache promotion), and result formatting (text/JSON/TOON). 94 tests (49 classifier + executor + formatter, 18 LLM integration, 27 caching).
- **Semantic Search CLI**: `tree-sitter search` command with `--search`, `--search-format`, `--search-no-cache`, and `--search-provider` options. Supports natural language queries like "functions named authenticate" or "all database in python files" with fast/slow path routing.
- **Semantic Search MCP Tool**: `semantic_search` MCP tool for AI-assisted code discovery. Provides semantic understanding for complex queries with LLM fallback, git SHA-based cache invalidation, and multiple output formats. Registered to query toolset (31 total tools).
- **Query Classifier**: `search/classifier.py` for distinguishing simple pattern queries from complex semantic queries. 9 fast path patterns (grep_by_name, grep_in_files, dependency_of, what_calls, functions_with_pattern, classes_with_method, all_tests, recently_changed, large_files) with confidence scoring and handler routing.
- **Fast Path Executor**: `search/executor.py` for high-performance query execution using ripgrep/fd/tree-sitter. Sub-second response time for simple queries with unified result format and tool attribution.
- **LLM Integration**: `search/llm_integration.py` for parsing natural language queries into tool calls. Multi-provider support (OpenAI GPT-4o-mini, Anthropic Claude 3.5 Haiku, Ollama, llama.cpp) with fallback and error handling.
- **Query Cache**: `search/cache.py` for git-aware result caching with TTL-based expiration. Git SHA invalidation, pattern learning (LLM → fast path promotion), and JSON persistence across sessions.
- **Search Result Formatter**: `search/formatter.py` for unified output formatting with error handling. Text, JSON, and TOON format support with metadata (query, execution_time, tool_used, cached).
...
- **Semantic Impact Tool**: `semantic_impact` MCP tool for semantic-level code impact analysis. Goes beyond textual occurrence counting to assess change risk. Considers type hierarchy (base class changes propagate to subclasses), visibility (public API vs private), call chain depth (deeper chains amplify blast radius), and API surface (interface/abstract members affect all implementations). Returns risk score (0-100) with SAFE/LOW/MODERATE/HIGH/CRITICAL classification, contributing factors, and actionable suggestions. Supports TOON format output. Registered to analysis toolset.
- **Quick Risk Assessment Tool**: `quick_risk_assessment` MCP tool for fast semantic-level risk assessment. Provides quick risk evaluation based on visibility, caller count, type hierarchy (base class/interface roots), and abstract modifiers. Suitable for rapid pre-commit checks. Registered to analysis toolset.
...
- **Tool Registration Module**: `mcp/tool_registration.py` registers all 27 MCP tools across 6 toolsets (analysis 🔍, query 🔎, navigation 🧭, safety 🛡️, diagnostic 🩺, index 📚).
- **Toolset Organization**: 6 toolsets group related tools by functionality — analysis (15 tools), query (3 tools), navigation (4 tools), safety (2 tools), diagnostic (2 tools), index (2 tools). Total: 28 MCP tools.
...
- **Dead Code Detection**: `analysis/dead_code.py` module for detecting unused code elements (functions, classes, imports). Includes `DeadCodeType` enum, `DeadCodeIssue` dataclass, `DeadCodeReport` dataclass, and helper functions (`is_entry_point`, `is_public_api`, `is_excluded_method`, `is_exported_symbol`, `is_test_file`). Language-specific exclusions for abstract methods, decorators (pytest.fixture, Flask/FastAPI routes), and test patterns.
- **Dead Code Tool**: `dead_code` MCP tool for detecting dead (unused) code. Identifies functions, classes, and imports that are defined but never referenced, with intelligent exclusions for entry points, test code, and public APIs. Supports three output formats: JSON (detailed), TOON (compact), summary (human-readable). Registered to analysis toolset.
- **Security Scanner Tool**: `security_scan` MCP tool for detecting security vulnerabilities. Scans source code for hardcoded secrets (API keys, passwords, tokens), SQL injection (string concatenation), command injection (subprocess calls), XSS (unsafe innerHTML/eval), unsafe deserialization (pickle/yaml loads), weak cryptography (MD5/SHA1), and path traversal (file operations with user input). Supports Python, JavaScript, TypeScript, Java, Go, C#, Ruby with severity filtering (critical/high/medium/low/info) and three output formats: TOON (default with emoji indicators), JSON (structured data), SARIF 2.1.0 (CI/CD integration with CWE mappings). Registered to safety toolset.
- **Test Coverage Analyzer**: `analysis/test_coverage.py` module for element-level test coverage analysis. Uses AST-based analysis to compare source code elements (functions, classes, methods) against test files. Supports Python, JavaScript, Java, Go with `SourceElement`, `TestCoverageResult`, and `TestCoverageAnalyzer` classes.
...
- **24 new Semantic Impact Tool tests**: `test_semantic_impact_tool.py` covering risk assessment (private/public/protected/abstract/interface), caller volume impact, static/constructor modifiers, deep call chains, framework annotations, TOON output format, validation, and risk scoring edge cases.
- **58 new Security Scanner tests**: `test_security_scan.py` (42 tests covering pattern detection, severity classification, false positive handling, multi-language support, CWE mapping, remediation advice) + `test_security_scan_tool.py` (16 tests covering MCP tool integration, output formats, validation, SARIF support).
