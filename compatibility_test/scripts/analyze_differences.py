...
class DifferenceAnalyzer:
    def __init__(self, version_a: str, version_b: str, project_root: str = None, config_path: str = None, smart_compare: bool = False, generate_normalized: bool = False):
        ...
        if self.smart_compare or self.generate_normalized:
            if config_path:
                ...
                self.config_path = self.project_root / "compatibility_test" / "config" / "comparison_config.json"
            
            if not self.config_path.exists():
                raise FileNotFoundError(f"比較設定ファイルが見つかりません: {self.config_path}")
            
            self.json_comparator = SmartJsonComparator(self.config_path)
            
            if self.generate_normalized:
                self.version_a_normalized_dir = self.results_dir / f"v{version_a}-normalized"
                self.version_b_normalized_dir = self.results_dir / f"v{version_b}-normalized"
    ...
    def _compare_json_structure(self, obj_a: Any, obj_b: Any, path: str) -> List[Dict[str, Any]]:
        """JSON構造を再帰的に比較"""
        differences = []
            ...
            for key in common_keys:
                ...
                if obj_a[key] != obj_b[key]:
                    if isinstance(obj_a[key], (dict, list)):
                        differences.extend(self._compare_json_structure(obj_a[key], obj_b[key], new_path))
                    else:
                        # 特定のキー名に基づいて重要度を判定
                        severity = self._determine_field_severity(key, obj_a[key], obj_b[key])
                        differences.append({
                            "type": "value_changed",
                            "path": new_path,
                            "old_value": obj_a[key],
            ...
            for i in range(min_length):
                ...
                if obj_a[i] != obj_b[i]:
                    if isinstance(obj_a[i], (dict, list)):
                        differences.extend(self._compare_json_structure(obj_a[i], obj_b[i], new_path))
                    else:
                        differences.append({
    ...
    def _determine_field_severity(self, field_name: str, old_value: Any, new_value: Any) -> str:
        """フィールド名と値に基づいて重要度を判定"""
        # 破壊的変更の可能性が高いフィールド
        breaking_fields = [
            "capture_name", "node_type", "name", "type", "id", 
            "start_line", "end_line", "start_column", "end_column"
        ]
        
        # パフォーマンス関連フィールド
        performance_fields = [
            "elapsed_ms", "execution_time", "fd_elapsed_ms", "rg_elapsed_ms",
    ...
    def _compare_performance_metrics(self, data_a: Any, data_b: Any) -> List[Dict[str, Any]]:
        """パフォーマンスメトリクスの変化を分析"""
        differences = []
    ...
    def analyze_all_differences(self) -> Dict[str, Any]:
        ...
        if self.generate_normalized:
            ...
            self.json_comparator.generate_normalized_files(self.version_a_dir, self.version_a_normalized_dir)
            print(f"  ...正規化ファイルを出力中: v{self.version_b}-normalized")
            self.json_comparator.generate_normalized_files(self.version_b_dir, self.version_b_normalized_dir)
            print("  ✅ 正規化ファイルの生成が完了しました。")
        
        if not self.version_a_dir.exists():
            print(f"❌ v{self.version_a} の結果ディレクトリが見つかりません: {self.version_a_dir}")
            return {}
        
        if not self.version_b_dir.exists():
            print(f"❌ v{self.version_b} の結果ディレクトリが見つかりません: {self.version_b_dir}")
            return {}
        ...
        for filename in sorted(common_files):
            ...
            # 統計を更新
            if analysis.get("type") == "identical" or analysis.get("is_identical_normalized"):
                ...
                if analysis.get("type") == "json_comparison":
                    perf_changes = [d for d in analysis.get("differences", [])
                    ...
                    if not analysis.get("is_identical_raw") and analysis.get("is_identical_normalized"):
                        results["summary"]["performance_changes"] += 1 # Or a more specific count
    ...
    def generate_analysis_report(self, analysis_results: Dict[str, Any]) -> str:
        ...
        report_name = "smart_comparison_report" if self.smart_compare else f"difference_analysis_{self.version_a}_vs_{self.version_b}"
        report_file = self.project_root / "compatibility_test" / "reports" / f"{report_name}_{timestamp}.md"
                ...
                if analysis["is_identical_normalized"]:
                    ...
                    if analysis["raw_diff"]:
                        ...
                        for key, values in ignored_diffs.items():
                             report_lines.append(f"- {key}: {values['old_value']} vs {values['new_value']}")
                else:
                    report_lines.append("- **Raw比較**: 差異あり")
                ...
                if differences:
                    ...
                    if high_severity:
                        ...
                        for diff in high_severity:
                            report_lines.append(f"- **{diff['type']}**: `{diff['path']}`")
                            if 'old_value' in diff and 'new_value' in diff:
                                report_lines.append(f"  - 変更前: `{diff['old_value']}`")
                                report_lines.append(f"  - 変更後: `{diff['new_value']}`")
                        report_lines.append("")
                    ...
                    if low_severity:
                        ...
                        for diff in low_severity:
                            if diff['type'] == 'performance_change':
                                ...
                                report_lines.append(f"- **パフォーマンス{direction}**: `{diff['path']}` ({change_pct:+.1f}%)")
                            else:
                                report_lines.append(f"- **{diff['type']}**: `{diff['path']}`")
                        report_lines.append("")
                else:
...
def main():
    ...
    parser.add_argument("--smart-compare", action="store_true", help="スマート比較モードを有効化")
    parser.add_argument("--generate-normalized", action="store_true", help="正規化されたJSONファイルを出力")
    ...
    try:
        ...
        print(f"📊 破壊的変更: {summary['breaking_changes']}, 非破壊的変更: {summary['non_breaking_changes']}")
