...
class StandardizedCompatibilityTester:
    def __init__(self, version_a: str, version_b: str, project_root: Optional[str] = None, clear_cache: bool = True):
        self.version_a = version_a
        self.version_b = version_b
        self.project_root = Path(project_root) if project_root else Path(__file__).parent.parent.parent
        self.compatibility_test_dir = self.project_root / "compatibility_test"
        self.mcp_settings_path = Path.home() / "AppData/Roaming/Cursor/User/globalStorage/rooveterinaryinc.roo-cline/settings/mcp_settings.json"
    ...
    def _cleanup_test_directories(self):
        ...
        # クリーンアップ結果の表示
        total_cleaned = cleanup_stats["results_cleaned"] + cleanup_stats["reports_cleaned"]
        if total_cleaned > 0:
            print(f"✅ {total_cleaned} 個のファイル/ディレクトリをクリーンアップしました")
        else:
            print(f"✅ クリーンアップ対象なし（既にクリーンな状態）")
        
        if cleanup_stats["errors"]:
            print(f"⚠️ クリーンアップ中に {len(cleanup_stats['errors'])} 件のエラーが発生しました")
    ...
    def load_mcp_settings(self) -> Dict[str, Any]:
        ...
        try:
            with open(self.mcp_settings_path, 'r', encoding='utf-8') as f:
                return json.load(f)
        except Exception as e:
    ...
    async def execute_test_case(self, tool_name: str, test_case: Dict[str, Any], output_dir: Path) -> Dict[str, Any]:
        """個別のテストケースを実行"""
    ...
    async def run_version_tests(self, version: str, output_dir: Path) -> Dict[str, Any]:
        ...
        if self.clear_cache:
            ...
            print(f"✅ {cache_clear_result['total_cleared']} 個のキャッシュをクリアしました")
        ...
        if self.clear_cache:
            ...
            cache_clear_result = self.cache_manager.clear_all_caches()
            print(f"✅ 追加で {cache_clear_result['total_cleared']} 個のキャッシュをクリアしました")
    ...
    def compare_outputs(self) -> Dict[str, Any]:
        ...
        if self.version_a_dir.exists():
            version_a_files = {f.name for f in self.version_a_dir.iterdir() if f.is_file()}
        
        # バージョンBの出力ファイル一覧を取得
        version_b_files = set()
        if self.version_b_dir.exists():
            version_b_files = {f.name for f in self.version_b_dir.iterdir() if f.is_file()}
        ...
        for filename in missing_in_b:
            comparison_results["missing_files"].append(f"{filename} (v{self.version_b}で欠落)")
        for filename in missing_in_a:
            comparison_results["missing_files"].append(f"{filename} (v{self.version_a}で欠落)")
        
        return comparison_results
    
    def generate_report(self, version_a_results: Dict[str, Any], version_b_results: Dict[str, Any], 
                       comparison_results: Dict[str, Any]) -> str:
        """比較レポートを生成"""
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        report_file = self.compatibility_test_dir / "reports" / f"comparison_report_{self.version_a}_vs_{self.version_b}_{timestamp}.md"
        
        # テンプレートを読み込み
        template_file = self.compatibility_test_dir / "templates" / "comparison_report_template.md"
        ...
        replacements = {
            ...
            "{SUMMARY}": self._generate_summary(version_a_results, version_b_results, comparison_results),
            "{TOTAL_TEST_CASES}": str(version_a_results["summary"]["total_tests"]),
            "{SUCCESSFUL_CASES}": str(min(version_a_results["summary"]["successful_tests"],
                                        version_b_results["summary"]["successful_tests"])),
            "{FAILED_CASES}": str(max(version_a_results["summary"]["failed_tests"],
                                    version_b_results["summary"]["failed_tests"])),
            "{DIFF_DETECTED_CASES}": str(len(comparison_results["different_files"])),
            "{VERSION_A_SERVER_STATUS}": "成功" if version_a_results["status"] == "success" else "失敗",
            "{VERSION_B_SERVER_STATUS}": "成功" if version_b_results["status"] == "success" else "失敗",
            "{CACHE_REPORT}": cache_report_content,
            "{IDENTICAL_ITEMS_LIST}": self._format_identical_items(comparison_results["identical_files"]),
            "{DETAILED_DIFFS}": self._format_detailed_diffs(comparison_results["detailed_diffs"]),
            "{PROJECT_ROOT}": str(self.project_root)
        }
    ...
    def _generate_summary(self, version_a_results: Dict[str, Any], version_b_results: Dict[str, Any], 
                         comparison_results: Dict[str, Any]) -> str:
        """総評を生成"""
        total_files = len(comparison_results["identical_files"]) + len(comparison_results["different_files"])
    ...
    def _generate_cache_report_content(self) -> str:
        ...
        try:
            ...
            # セクション番号を調整
            cache_content = cache_content.replace("## 🧹 キャッシュ状態レポート", "## 2.3. キャッシュ状態")
    ...
    async def run_compatibility_test(self) -> bool:
        ...
        try:
            ...
            # バージョンAのテスト実行
            version_a_results = await self.run_version_tests(self.version_a, self.version_a_dir)
            
            # バージョンBのテスト実行
            version_b_results = await self.run_version_tests(self.version_b, self.version_b_dir)
            ...
            # レポート生成
            report_file = self.generate_report(version_a_results, version_b_results, comparison_results)
            ...
            if self.clear_cache:
                print(f"🧹 キャッシュクリア実行回数: 各バージョンで2回ずつ")
...
async def main():
    ...
    parser.add_argument("--version-a", required=True, help="比較元バージョン (例: 1.9.2)")
    parser.add_argument("--version-b", required=True, help="比較先バージョン (例: 1.9.3)")
    parser.add_argument("--project-root", help="プロジェクトルートパス (デフォルト: 自動検出)")
    parser.add_argument("--no-cache-clear", action="store_true", help="キャッシュクリアを無効化（デバッグ用）")
