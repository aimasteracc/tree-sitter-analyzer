...
class SmartJsonComparator:
    ...
    def _sort_arrays(self, data):
        ...
        if isinstance(data, dict):
            for key, value in data.items():
                if key in self.config.get("sort_arrays_by", {}) and isinstance(value, list):
                    sort_key = self.config["sort_arrays_by"][key]
                    # sort_keyが存在する要素のみをソート対象とする
                    items_to_sort = [item for item in value if isinstance(item, dict) and sort_key in item]
                    # sort_keyが存在しない要素はそのまま
                    other_items = [item for item in value if not (isinstance(item, dict) and sort_key in item)]
    ...
    def normalize_json(self, data):
        ...
        def recursive_normalize(d):
            if isinstance(d, dict):
                new_dict = {}
                # キーの正規化が有効な場合はキーでソート
                items = sorted(d.items()) if self.config.get("normalize_keys") else d.items()
                for k, v in items:
    ...
    def compare_with_report(self, file1_path, file2_path):
        ...
        raw_diff = deepdiff.DeepDiff(data1, data2, ignore_order=self.config.get("ignore_field_order", True))
        normalized_diff = deepdiff.DeepDiff(normalized1, normalized2, ignore_order=self.config.get("ignore_field_order", True))
