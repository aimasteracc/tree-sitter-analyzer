...
class CacheReporter:
    ...
    def _generate_summary(self, cache_stats: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "analysis_engine" in cache_stats and not cache_stats["analysis_engine"].get("error"):
            ae_stats = cache_stats["analysis_engine"]
            summary["total_cache_systems"] += 1
            
            if ae_stats.get("l1_size", 0) > 0 or ae_stats.get("l2_size", 0) > 0 or ae_stats.get("l3_size", 0) > 0:
                summary["active_caches"] += 1
                summary["total_cached_items"] += ae_stats.get("l1_size", 0) + ae_stats.get("l2_size", 0) + ae_stats.get("l3_size", 0)
            
            if "hit_rate" in ae_stats:
                summary["cache_hit_rates"]["analysis_engine"] = ae_stats["hit_rate"]
        
        # Search Content キャッシュ
        if "search_content" in cache_stats and not cache_stats["search_content"].get("error"):
            ...
            if "hit_rate_percent" in sc_stats:
                summary["cache_hit_rates"]["search_content"] = sc_stats["hit_rate_percent"]
        
        # 潜在的な問題を特定
        if summary["active_caches"] > 0:
            summary["potential_issues"].append("アクティブなキャッシュが検出されました - テスト結果に影響する可能性があります")
        
        for cache_name, hit_rate in summary["cache_hit_rates"].items():
            if hit_rate > 50:  # 50%以上のヒット率
                summary["potential_issues"].append(f"{cache_name}のヒット率が高い ({hit_rate}%) - バージョン間でキャッシュが共有されている可能性があります")
    ...
    def _generate_recommendations(self, cache_stats: Dict[str, Any]) -> List[str]:
        ...
        if active_caches > 0:
            recommendations.append("🚨 アクティブなキャッシュが検出されました。テスト前にキャッシュをクリアすることを強く推奨します。")
            recommendations.append("💡 --no-cache-clear オプションを使用せず、デフォルトのキャッシュクリア機能を有効にしてください。")
        ...
        for cache_name in ["analysis_engine", "search_content"]:
            if cache_name in cache_stats and not cache_stats[cache_name].get("error"):
                ...
                hit_rate_key = "hit_rate" if cache_name == "analysis_engine" else "hit_rate_percent"
                hit_rate = stats.get(hit_rate_key, 0)
                
                if hit_rate > 30:  # 30%以上のヒット率
                    recommendations.append(f"⚠️ {cache_name}のキャッシュヒット率が高い ({hit_rate}%) - バージョン間でキャッシュが共有されている可能性があります。")
    ...
    def _analyze_cache_impact(self, cache_stats: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "analysis_engine" in cache_stats and not cache_stats["analysis_engine"].get("error"):
            ae_stats = cache_stats["analysis_engine"]
            total_items = ae_stats.get("l1_size", 0) + ae_stats.get("l2_size", 0) + ae_stats.get("l3_size", 0)
            hit_rate = ae_stats.get("hit_rate", 0)
            
            max_risk_score += 0.4
            if total_items > 0:
                risk_score += 0.2
                impact_analysis["risk_factors"].append(f"Analysis Engineに{total_items}個のキャッシュエントリが存在")
            
            if hit_rate > 0.3:  # 30%以上
                risk_score += 0.2
                impact_analysis["risk_factors"].append(f"Analysis Engineのヒット率が高い ({hit_rate:.1%})")
        ...
        if "search_content" in cache_stats and not cache_stats["search_content"].get("error"):
            ...
            if size > 0:
                risk_score += 0.3
                impact_analysis["risk_factors"].append(f"Search Contentに{size}個のキャッシュエントリが存在")
            
            if hit_rate > 0.3:  # 30%以上
                risk_score += 0.3
                impact_analysis["risk_factors"].append(f"Search Contentのヒット率が高い ({hit_rate:.1%})")
...
def generate_cache_report(project_root: Optional[str] = None) -> Dict[str, Any]:
    """
    便利関数：キャッシュレポートを生成
