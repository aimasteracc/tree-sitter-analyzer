...
| `query` | `file_path`, `filter`, `find_references`, `language`, `max_count`, `output_file`, `output_format`, `query_key`, `query_string`, `result_format`, `suppress_output`, `symbol`, `symbol_type` | `success`*, `verdict`*, `agent_summary`, `error` + action payload | `--query-key` |
