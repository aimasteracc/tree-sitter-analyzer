...
**Design**: デフォルトは **scale-only**（軽量出力）。`include_structure=true` で詳細な structural_overview を取得可能。
...
```json
{
  ...
  "properties": {
    ...
    "include_structure": {
      "type": "boolean",
      "description": "classes/methods/fields の完全な structural_overview を含める。デフォルト false で scale-only（トークン節約）",
      "default": false
    },
    "output_format": {
      "type": "string",
      "enum": ["json", "toon"],
      "description": "出力形式。toon で約50-70%トークン削減",
      "default": "toon"
    }
  },
  "required": ["file_path"]
...
デフォルトは `output_format="toon"`。応答形式により構造が異なる。

**TOON形式**（デフォルト、`output_format="toon"`）:
```json
{
  "format": "toon",
  "toon_content": "<TOON記法の文字列: file_metrics, summary, llm_guidance 等を約50-70%トークン削減でシリアライズ>",
  "success": true,
  "file_path": "path/to/file.py",
  "language": "python"
}
```
- `toon_content`: 本体データを TOON 記法でエンコードした文字列。`docs/toon-format-guide.md` を参照。
- メタデータ（success, file_path, language）はスカラーとして個別に保持。

**JSON形式**（`output_format="json"` 指定時）:
```json
{
  "success": true,
  "file_path": "path/to/file.py",
...
```
※ `structural_overview`（classes, methods, fields の詳細）は `include_structure=true` の場合のみ含まれる。

**Token Optimization**: デフォルトは TOON + scale-only（`include_structure=false`）で最大限トークン削減。詳細が必要な場合は `include_structure=true` または `analyze_code_structure` を使用。
