...
- **`set_project_path` removed**: project root is now configured once at server startup (no longer a runtime tool)
...
```
**Solution**: サーバー起動時に `TREE_SITTER_PROJECT_ROOT` 環境変数または `--project-root` フラグでプロジェクトルートを正しく設定（v1.13.0 で `set_project_path` ランタイムツールは廃止）。
...
{
  "mode": "record",
  "title": "Drop set_project_path runtime tool",
  "rationale": "Project root is fixed at server startup; runtime mutation broke security validator caching.",
  "verdict": "SAFE",
  "scope_paths": ["tree_sitter_analyzer/mcp/server.py"],
  "alternatives": ["keep with stricter path validation"],
