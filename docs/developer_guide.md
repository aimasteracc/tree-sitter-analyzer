...
`tree-sitter-analyzer` は、パフォーマンスを最適化するために2つの主要なキャッシュシステムを実装しています。

1.  **`UnifiedAnalysisEngine` キャッシュ (`core/cache_service.py`)**
    *   **目的**: `tree-sitter`によるコード解析結果（AST、シンボルテーブル等）をキャッシュします。
    *   **特徴**: L1, L2, L3の3層階層キャッシュを採用し、高速アクセスとデータ保持期間のバランスを取っています。これはシステムのコア機能であり、解析処理全体のパフォーマンスに寄与します。

2.  **`SearchContentTool` キャッシュ (`mcp/utils/search_cache.py`)**
    *   **目的**: `ripgrep`を利用したファイル内容検索（`search_content`ツール）の実行結果をキャッシュします。
    *   **特徴**: LRU (Least Recently Used) と TTL (Time To Live) を組み合わせた、特定のツールに特化したキャッシュです。

**なぜ2つ存在するのか？**
この2つのキャッシュは、役割とライフサイクルが異なるため、分離して実装されています。`UnifiedAnalysisEngine`キャッシュは汎用的な**解析データ**を扱い、`SearchContentTool`キャッシュは特定の**ツール実行結果**を扱います。この分離設計により、それぞれのユースケースに最適化されたキャッシュ戦略を適用でき、疎結合なアーキテクチャを維持しています。

開発者は、新しいツールを実装する際に、処理結果が再利用可能であれば、同様のツールレベルのキャッシュを導入することを検討できます。ただし、互換性テストの信頼性を確保するため、テスト実行時にはこれらのキャッシュが適切にクリアされることを保証する必要があります。

**詳細な設計分析については、[キャッシュシステム設計分析](CACHE_SYSTEM_ANALYSIS.md)を参照してください。**
