...
| `edit` | safe / guard / impact / refactor / constraints / pr / classify / ast_diff | Edit-safety, blast-radius, refactor, PR review; RFC-0022 explicit-opt-in frozen workspace/staged snapshots (`impact` with `capture_diff_snapshot=true` issues ID+lease, `ast_diff`/`classify` consume ID+path) |
