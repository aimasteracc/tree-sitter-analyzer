...
| `health` | project / file / scale / patterns / heatmap / imports / matrix / dead / routes / overview / deps / test_gap / self | Code health, complexity, dependency analysis, untested symbol discovery; `self` = RFC-0025 Layer 5 self-proprioception (per-`(tool, action)` p50/p95 latency by tier + AST-cache hit rate for the current process; CLI twin `--self-health`) |
