...
| `edit` | safe / guard / impact / refactor / constraints / pr / classify / ast_diff / release_snapshot | Edit-safety, blast-radius, refactor, PR review; RFC-0022 explicit-opt-in frozen workspace/staged snapshots (`impact` issues ID+lease, `ast_diff`/`classify` consume ID+path, `release_snapshot` idempotently closes the owned lease in the same MCP process) |
