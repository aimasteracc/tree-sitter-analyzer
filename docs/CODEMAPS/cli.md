...
- `--diff-snapshot-id ID` with `--ast-diff-file` or `--semantic-classify` — consume frozen bytes without rereading/reconstructing the workspace; legacy file/string/git-ref modes remain compatible
