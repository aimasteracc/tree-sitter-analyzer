...
│                     言語プラグイン層                            │
├────────┬────────┬────────┬────────┬────────┬────────────────┤
│ Python │ Java   │ JS/TS  │ C/C++  │ Go     │ その他 (10+)    │
│ Plugin │ Plugin │ Plugin │ Plugin │ Plugin │ Ruby, Rust...  │
└────────┴────────┴────────┴────────┴────────┴────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────┐
│                     データ管理層                               │
...
class PluginManager:
    ...
    def get_plugin(self, language: str) -> LanguagePlugin | None:
        """言語に対応するプラグインを取得"""
        return self._plugins.get(language.lower())
    ...
    def _load_builtin_plugins(self) -> None:
        ...
        self.register_plugin("python", PythonPlugin())
        self.register_plugin("java", JavaPlugin())
```
...
| 言語 | プラグインクラス | tree-sitterライブラリ | 状態 |
|------|----------------|---------------------|------|
...
```
[ユーザー] 
    ↓ (1) tree-sitter-analyzer analyze file.py
[CLI/MCPインターフェース]
    ↓ (2) api.analyze_file()
[API Facade]
