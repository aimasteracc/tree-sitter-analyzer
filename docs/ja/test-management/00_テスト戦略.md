...
**カバー範囲:**
- 全MCPツール（2実装）:
  - **公開版** (`tree_sitter_analyzer.mcp.server` - PyPI配布中, 8種類): check_code_scale, analyze_code_structure, extract_code_section, set_project_path, query_code, list_files, search_content, find_and_grep
  - **新設計版** (`tree_sitter_analyzer.interfaces.mcp_server` - 開発中, 8種類): analyze_file, analyze_code, extract_elements, execute_query, validate_file, get_supported_languages, get_available_queries, get_framework_info
- 全CLIコマンド（7種類: DefaultCommand, QueryCommand, TableCommand, AdvancedCommand, SummaryCommand, StructureCommand, PartialReadCommand）
...
| **毎PR** | 全テスト（3プラットフォーム） | 15-30分 | GitHub Actions（Ubuntu, Windows, macOS） |
