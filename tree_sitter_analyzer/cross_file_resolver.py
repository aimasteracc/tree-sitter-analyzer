...
class CrossFileResolver:
    ...
    def resolve_callee(
        ...
        target_file = name_sources.get(base_name) or name_sources.get(dot_parts[0] if len(dot_parts) == 2 else base_name)
    ...
    def find_caller_function(
        ...
        for func in funcs:
            if func.line <= call_line <= func.end_line:
                if best is None or (func.end_line - func.line) < (best.end_line - best.line):
                    best = func
        if best is not None:
