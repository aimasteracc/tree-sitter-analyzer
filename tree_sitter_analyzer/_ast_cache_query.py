...
def search_symbols_cascade(
    ...
    """Three-tier cascading symbol search with result merging.

    Tier 1: Exact name match (SQL =) — highest relevance.
    Tier 2: FTS5 BM25 ranked — medium relevance.
    Tier 3: LIKE substring — lowest relevance (fallback).

    Results from each tier are merged with deduplication by (name, file, line).
    Each result includes a ``relevance_score`` and ``match_tier`` field.
    """
    if len(query) < 1:
        return []
    ...
    if fts5_available and len(query) >= 2:
        fts_results = fts_search_ranked(conn, query, language, limit * 2)
        for r in fts_results:
            key = (r["name"], r["file"], r.get("line", 0))
            if key not in seen:
                seen.add(key)
                results.append(
                    {
                        **r,
                        "relevance_score": r.get("relevance_score", 0.5) * 0.8,
                        "match_tier": "fts5",
                    }
                )
