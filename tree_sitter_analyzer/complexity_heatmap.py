...
def _extract_functions(tree: Any, source: str, language: str, file_path: str) -> list[FunctionComplexity]:
...
def analyze_file_complexity_from_cache(
    ...
    if row is None:
        return analyze_file_complexity(file_path, _language_from_ext(file_path) or "python")
...
def analyze_project_heatmap(
    ...
    source_files = _collect_source_files(project_root, language_filter, directory_filter, max_files)
        ...
        "file_heatmaps": [
            {
                ...
                "top_functions": [
                    ...
                    for fc in sorted(fh.functions, key=lambda x: x.complexity, reverse=True)[:5]
                ],
            }
...
def _collect_source_files(
    ...
    skip_dirs = {
        ...
        ".pytest_cache", ".ruff_cache", ".venv", "venv", ".eggs",
        "dist", "build", ".ast-cache", ".tree-sitter-cache",
        ".swarm", ".claude-flow", ".claude",
    }
