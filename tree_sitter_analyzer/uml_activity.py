...
def _condition_text(node: Any) -> str:
    """Extract condition text from an if/while node (first non-body child)."""
...
def _find_function_node(root: Any, function_name: str) -> Any | None:
    """DFS search for a function_definition node named *function_name*."""
    if root is None:
        return None
...
class _CFGWalker:
    ...
    def _handle_statement(
        ...
        if kind == "return_statement":
            ret = self._add_node("return " + _node_text(node, 20), "return")
            if ret is not None:
                for pred in incoming:
                    self._add_edge(pred, ret)
            return []  # terminal: no outgoing paths
        if kind == "raise_statement":
            exc_text = _node_text(node, 20)
            raise_node = self._add_node("raise " + exc_text, "raise")
    ...
    def _handle_try(self, node: Any, incoming: list[CFGNode]) -> list[CFGNode]:
        ...
        return outgoing if outgoing else [try_node]
