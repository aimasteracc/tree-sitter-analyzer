...
def _format_trace_summary(data: dict[str, Any]) -> str:
    ...
    if defs:
        ...
        for d in defs:
            ...
            lines.append(
                f"  {d.get('file_path', '?')}:{d.get('line', '?')}  {d.get('symbol_type', '?')} {d.get('name', '?')}({params})"
            )
    ...
    if usages:
        ...
        for u in usages:
            lines.append(
                f"  {u.get('file_path', '?')}:{u.get('line', '?')}  [{u.get('ref_type', '?')}] {u.get('context_function', '?')}"
            )
    ...
    if callers:
        ...
        for c in callers:
            lines.append(
                f"  {c.get('caller_file', '?')}:{c.get('line', '?')}  {c.get('caller_function', '?')}()"
            )
    if callees:
        lines.append(f"\nCallees ({len(callees)}):")
        for c in callees:
...
def _format_trace_tree(data: dict[str, Any]) -> str:
    ...
    for d in data.get("definitions", []):
        lines.append(
            f"def:{d.get('file_path', '?')}:{d.get('line', '?')}:{d.get('symbol_type', '?')}"
        )
    for u in data.get("usages", []):
        lines.append(
            f"ref:{u.get('file_path', '?')}:{u.get('line', '?')}:{u.get('ref_type', '?')}"
        )
    cc = data.get("call_chain", {})
...
def _format_impact_summary(data: dict[str, Any]) -> str:
    ...
    if direct:
        ...
        for d in direct:
            lines.append(
                f"  {d.get('file_path', '?')}:{d.get('line', '?')}  {d.get('symbol_name', '?')} [{d.get('impact_type', '?')}]"
            )
    ...
    if trans:
        ...
        for t in trans:
            lines.append(
                f"  {t.get('file_path', '?')}:{t.get('line', '?')}  {t.get('symbol_name', '?')} [depth={t.get('depth', '?')}]"
            )
...
def format_architecture_report(
    data: dict[str, Any], output_format: str = "summary"
) -> str:
    if output_format == "json":
        return json.dumps(data, indent=2, ensure_ascii=False)
...
def _format_arch_summary(data: dict[str, Any]) -> str:
    ...
    if cycles:
        ...
        for c in cycles:
            lines.append(
                f"  [{c.get('severity', '?')}] {' -> '.join(c.get('files', []))}"
            )
    ...
    if violations:
        ...
        for v in violations:
            lines.append(
                f"  {v.get('source_file', '?')} -> {v.get('target_file', '?')}: {v.get('description', '')}"
            )
    ...
    if gods:
        ...
        for g in gods:
            lines.append(
                f"  {g.get('class_name', '?')} ({g.get('file_path', '?')}): {g.get('method_count', 0)} methods"
            )
    ...
    if metrics:
        ...
        for name, m in metrics.items():
            lines.append(
                f"  {name}: I={m.get('instability', 0):.2f} A={m.get('abstractness', 0):.2f} D={m.get('distance_from_main_sequence', 0):.2f}"
            )
    ...
    if tc:
        ...
        if untested:
            ...
            for s in untested[:20]:  # cap display
                lines.append(
                    f"    {s.get('file_path', '?')}:{s.get('line', '?')} {s.get('symbol_type', '?')} {s.get('name', '?')}"
                )
            if len(untested) > 20:
        ...
        if over:
            ...
            for s in over[:10]:
                lines.append(
                    f"    {s.get('name', '?')} ({s.get('test_ref_count', 0)} test refs)"
                )
