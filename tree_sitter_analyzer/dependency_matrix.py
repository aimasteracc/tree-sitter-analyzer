...
class DependencyMatrixResult:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "high_coupling_pairs": [
                e.to_dict() for e in self.high_coupling_pairs
            ],
...
class DependencyMatrix:
    ...
    def build(self) -> DependencyMatrixResult:
        ...
        self._modules = set(self._import_edges.keys()) | set(
            self._call_edges.keys()
        )
        ...
        for source_file, imports in imports_by_file.items():
            ...
            for imp_text in imports:
                ...
                resolved = self._resolve_import(
                    imp_text, source_file, project_files
                )
    ...
    def _resolve_import(
        ...
        if import_text.startswith("from ") or import_text.startswith(
            "import "
        ):
            return self._resolve_python_import(
                import_text, source_file, project_files
            )
        return None
    ...
    def _compute_matrix(self) -> DependencyMatrixResult:
        ...
        for entry in coupling_map.values():
            entry.score = (
                entry.import_count * IMPORT_FACTOR
                + entry.call_count * CALL_FACTOR
            )
        ...
        result.high_coupling_pairs = [
            e
            for e in result.coupling_pairs
            if e.score >= HIGH_COUPLING_THRESHOLD
        ]
    ...
    def _compute_module_stats(
        ...
        for mod in modules:
            afferent = sum(
                1
                for src in self._import_edges
                if mod in self._import_edges[src]
            )
    ...
    def unstable_modules(self, threshold: float = 0.7) -> list[ModuleStats]:
        ...
        return [
            s
            for s in self._result.module_stats
            if s.instability >= threshold
        ]
    ...
    def summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "avg_coupling": round(sum(scores) / len(scores), 2)
            if scores
            else 0.0,
