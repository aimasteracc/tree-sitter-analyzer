...
def symbol_projection_is_exact(conn: sqlite3.Connection, max_symbols: int) -> bool:
    """Verify marker evidence without parsing any legacy JSON payload."""
    ...
    checks = (
        ...
        f"WHERE state.file_path IS NULL OR state.content_hash IS NOT {hash_sql} LIMIT 1",
        ...
        "SELECT 1 FROM ast_symbol_projection_state AS state LEFT JOIN "
        "(SELECT file_path, COUNT(*) AS actual FROM ast_symbol_rows GROUP BY file_path) "
        "AS rows ON rows.file_path=state.file_path "
        "WHERE typeof(state.file_path)!='text' OR typeof(state.content_hash)!='text' "
        "OR typeof(state.symbol_count)!='integer' OR state.symbol_count < 0 "
        f"OR state.symbol_count > {max_symbols} "
        "OR state.symbol_count != COALESCE(rows.actual, 0) LIMIT 1",
        "SELECT 1 FROM (SELECT file_path FROM ast_symbol_rows GROUP BY file_path) AS rows "
        "LEFT JOIN ast_symbol_projection_state AS state "
        "ON state.file_path=rows.file_path WHERE state.file_path IS NULL LIMIT 1",
    )
