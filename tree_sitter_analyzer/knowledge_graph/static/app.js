...
const edgeColors = {calls:"#fb4667", imports:"#38bdf8", extends:"#9b7cff", implements:"#b9a2ff", doc_links:"#f5a524", contains:"#64748b"};
...
function setGraph(graph) {
  ...
  layout(); renderExplorer(); renderMetrics(); resize(); fit(); heat();
}
...
function renderExplorer() {
  ...
  const files = state.files
    .filter(n => {
      ...
      return !q || p.toLowerCase().includes(q) || String(a.kind || "").includes(q);
    });
  ...
  const activeExplorerKey = state.activeFile || state.selected;
  if (!q && activeExplorerKey) {
    const activeGroup = sortedGroups.find(([, items]) => items.some(item => item.key === activeExplorerKey));
    if (activeGroup && !visibleGroups.includes(activeGroup)) visibleGroups = [activeGroup, ...visibleGroups.slice(0,19)];
  }
  $("explorer").innerHTML = visibleGroups
    .map(([dir, items], i) => {
      const sortedItems = items.sort((a,b)=>a.path.localeCompare(b.path));
      const activeItem = activeExplorerKey ? sortedItems.find(item => item.key === activeExplorerKey) : null;
      let visibleItems = sortedItems.slice(0,90);
      if (activeItem && !visibleItems.includes(activeItem)) visibleItems = [activeItem, ...visibleItems.slice(0,89)];
      return `<details ${i<4?"open":""}><summary>${esc(dir)} <small>${items.length}</small></summary>${visibleItems.map(item => {
      const label = item.path.split("/").slice(1).join("/") || item.path.split("/").pop();
      const active = item.key === activeExplorerKey ? " activeFile" : "";
      return `<button class="${active}" data-node="${esc(item.key)}" data-file="${esc(item.path)}">${esc(label)}</button>`;
    }).join("")}</details>`;
    })
    .join("") || "<p>No files match this filter.</p>";
}
...
function layout() {
  ...
  ordered.forEach((items, gi) => {
    ...
    items.forEach((n, i) => {
      ...
      a.gx = a.x; a.gy = a.y;
      a.vx = 0; a.vy = 0; a.pinned = false; a.r = Math.max(4, Number(a.size || 4) + (state.adjacent.get(n.key)?.size ? 2 : 0));
    });
  });
  state.alpha = .85;
}
function applyLocalLayout(centerId) {
  ...
  const golden = Math.PI * (3 - Math.sqrt(5));
  const directNodes = [...direct.keys()]
    .map(key => state.byKey.get(key))
    .filter(Boolean)
    .sort((a,b) => {
      const da = direct.get(a.key), db = direct.get(b.key);
      return String(attr(da.edge).kind).localeCompare(String(attr(db.edge).kind))
        || da.direction.localeCompare(db.direction)
        || String(attr(a).label || a.key).localeCompare(String(attr(b).label || b.key));
    });
  ...
  const outerNodes = state.nodes
    .filter(node => node.key !== centerId && !direct.has(node.key))
    .sort((a,b) => String(attr(a).label || a.key).localeCompare(String(attr(b).label || b.key)));
...
function tick() {
  ...
  for (const n of nodes) {
    const a = attr(n);
    if (!a.pinned) { a.vx += ((a.gx||0)-a.x)*.0018*state.alpha; a.vy += ((a.gy||0)-a.y)*.0018*state.alpha; }
  }
...
function draw() {
  ...
  for (const n of state.nodes) {
    const a=attr(n), [x,y]=screen(a.x,a.y), active=n.key===state.selected||n.key===state.hover||peers.has(n.key), r=Math.max(3.5,(a.r||5)*Math.sqrt(state.scale));
    ctx.globalAlpha = focus && !active ? .24 : 1; ctx.fillStyle = colors[a.kind] || "#94a3b8"; ctx.strokeStyle = n.key===state.selected ? "#fff" : "#071015"; ctx.lineWidth = active ? 2.6 : 1.2;
    ...
    if (showLabel) { ctx.fillStyle = (n.key===state.selected || n.key===state.hover) ? "#fff" : "#cbd5e1"; ctx.font = (n.key===state.selected || n.key===state.hover) ? "700 12px system-ui" : "11px system-ui"; ctx.fillText(String(a.label||n.key).slice(0,64), x+r+5, y+4); }
  }
  ctx.globalAlpha = 1;
}
...
async function inspect(id) {
  ...
  state.selected = id; const a = data.node; $("selectionTitle").textContent = a.label || id;
  ...
  $("details").innerHTML = kv + `<div class="edgeList">${edges || "<p>No relationships.</p>"}</div>`; draw();
}
...
canvas.addEventListener("click", e => { const [x,y]=pointer(e), n=pick(x,y); if (n) inspect(n.key); else { state.selected=null; syncSelectionActions(); $("selectionTitle").textContent="No selection"; $("details").innerHTML="<p>Select a node or choose a file.</p>"; draw(); } });
...
$("loadGraph").onclick = loadOverview; $("fitGraph").onclick = () => { fit(); draw(); };
...
$("fileFilter").addEventListener("input", renderExplorer);
$("explorer").onclick = e => { const b=e.target.closest("button[data-node]"); if(!b) return; state.activeFile=b.dataset.node; $("command").value=b.dataset.file; renderExplorer(); loadLocal(b.dataset.node); };
$("viewMode").onclick = e => { const b=e.target.closest("button[data-view]"); if(!b) return; state.view=b.dataset.view; syncViewButtons(); state.view==="overview" ? loadOverview() : state.selected && loadLocal(state.selected); };
$("lodMode").onclick = e => { const b=e.target.closest("button[data-lod]"); if(!b) return; state.lod=b.dataset.lod; document.querySelectorAll("[data-lod]").forEach(x=>x.classList.toggle("active",x===b)); loadOverview(); };
