...
## Session 170 — 2026-04-25: Constructor Side Effect Detector (1-in-1-out)

**类型**: 1-in-1-out (net zero)

**新增**: `constructor_side_effects.py` — Constructor Side Effect Detector
- 检测构造函数中的 I/O 操作、线程创建、网络调用、文件系统访问
- Issue types: `constructor_io` (high), `constructor_thread` (medium)
- 多语言支持: Python (__init__), JS/TS (constructor), Java (constructor_declaration), Go (func New*)
- 基于 Misko Hevery 的"不要在构造函数中做工作"原则

**移除**: `context_optimizer.py`
**原因**: 不是代码质量检测器（不继承 BaseAnalyzer），而是上下文窗口优化工具，属于基础设施而非分析引擎

**功能评分**: 10/12
- 竞品差距: 3/3 — 无 ESLint/Ruff/SonarQBE/Pylint 规则检测构造函数中的 I/O
- 用户信号: 2/3 — Misko Hevery 原则，常见 Code Review 反馈，测试痛点
- 架构适配: 3/3 — BaseAnalyzer, 方法级 AST 分析, 多语言
- 实现成本: 2/3 — 中等复杂度（构造函数节点识别 + I/O 调用检测）

**产品分析 (Steve Jobs Perspective)**:
- 问题：构造函数打开文件、发 HTTP 请求、创建线程时，创建了隐藏耦合，让类无法不 mock 基础设施来测试
- 一句话："Find constructors that reach outside their own class — the #1 reason your tests need mocks"
- 无竞品覆盖：ESLint 没有构造函数副作用规则，Ruff 没有，SonarQBE 也没有
- 用户价值：识别需要依赖注入重构的构造函数

**架构分析**:
- 组件：`tree_sitter_analyzer/analysis/constructor_side_effects.py`
- Base class: BaseAnalyzer
- 算法：1) 遍历 AST 找到构造函数节点 → 2) 在构造函数体内检测 I/O 调用 → 3) 根据调用类型报告
- 数据结构：`ConstructorSideEffectIssue` (frozen dataclass), `ConstructorSideEffectResult` (frozen dataclass)
...
| Exception Handling Pattern Analyzer (5-in-1) | error_handling, silent_suppression, exception_signature (ts-analyzer internal) | All 5 sub-features already covered by existing tools |
| Data Class Smell Detector | SonarQBE S2068 | SonarQBE covers Python+Java; 9/12 < 10/12 |
...
| Data Class Smell Detector | 9/12 | 2 | 2 | 3 | 2 | SonarQBE S2068 covers Python+Java; gap only in JS/TS+Go; user signal moderate |
| Loop Variable Mutation | — | — | — | — | — | Overlap with variable_mutability MUTABILITY_LOOP_MUTATION (already detects loop body modifying outer scope vars) |
...
## 产品讨论记录 - Exception Handling Pattern Analyzer - 2026-04-26 Session 28

**调用**: /steve-jobs-perspective

**输入**: Exception Handling Pattern Analyzer — 检测代码中的异常处理模式问题：
1. 捕获所有异常的裸 except 子句
2. 吞掉异常的空 catch 块
3. 缺少 finally 的资源清理
4. 函数可抛出异常但未文档化
5. 项目内不一致的错误处理风格
...
This is five features pretending to be one. That's the first problem. If you can't say it in one sentence, the idea isn't focused enough.

Sub-feature analysis:
1. **裸 except 子句** — Already covered by `error_handling.py` (bare_except, broad_exception). Redundant.
2. **空 catch 块** — Already implemented as `silent_suppression.py` (Session 167). Redundant.
3. **缺少 finally 的资源清理** — Ruff SIM115 covers unclosed files; SonarQBE S2765 covers resource management. The "missing finally" pattern is too ambiguous — which resources? Which patterns? No clear definition.
4. **函数可抛出异常但未文档化** — This is Exception Signature, already discussed and IMPLEMENTED (Session 162). Redundant.
5. **不一致的错误处理风格** — Project-level pattern detection. Doesn't fit single-file BaseAnalyzer model. Would need cross-file analysis.

**聚焦即说不**: 5 sub-ideas = insufficient focus. "Focus means saying no to a hundred other good ideas." All 5 are either covered by existing tools or don't fit the architecture.

**减法思维**: The existing tools (error_handling, silent_suppression, exception_signature, security_scan) already cover the critical exception handling issues. What's left is marginal.

**一句话定义**: Can't say it in one sentence — because it's 5 features, not 1. That's the answer.

**结论**: DON'T — 全部 5 个子功能已被现有工具覆盖或已实现，或不符合单文件 AST 分析器模式。

**理由**: Redundant with existing analyzers (error_handling, silent_suppression, exception_signature). Project-level pattern detection doesn't fit BaseAnalyzer. No new value.

**被拒绝功能注册表更新**: Exception Handling Pattern Analyzer — 5 个子功能全部已被现有工具覆盖
