...
> 预建 AST 索引 + 59 个 MCP 工具 + 13 个精选 agent 技能 + TOON 压缩输出。
...
每个 skill 都带 `allowed-tools` 工具子集 + 操作流程 + 决策面 schema，agent 不必在 59 个工具间反复挑选。
...
索引首次查询时懒构建，文件变更时通过内容哈希增量刷新（`codegraph_incremental_sync`）。所有 59 个工具共享同一份 `.ast-cache/`，查询与跟进调用共享工作量。
