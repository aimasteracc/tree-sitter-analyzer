...
[![测试](https://img.shields.io/badge/tests-4844%20passed-brightgreen.svg)](#质量保证)
...
[![版本](https://img.shields.io/badge/version-1.9.16-blue.svg)](https://github.com/aimasteracc/tree-sitter-analyzer/releases)
...
**完整的YAML语言支持**即将在下一版本发布：

- **结构提取**：映射（键值对）、序列（列表）、标量值
- **高级功能**：
  - 锚点（&anchor）和别名（*alias）检测
  - 多文档支持（---分隔符）
  - 注释提取
  - 嵌套层级计算
- **标量类型识别**：自动识别字符串、数字、布尔值、null
- **YAML格式化器**：专用输出格式化器，支持摘要、结构、高级分析、表格格式
- **Tree-sitter查询支持**：复杂YAML模式分析
- **完全集成**：CLI、API和MCP接口均可使用

完美支持DevOps工程师和使用Kubernetes、Docker Compose、Ansible等YAML配置文件的开发者！
...
- **SQL单行解析**：修复视图定义解析问题
- **SQL触发器行号提取**：修复并清理冗余的golden master文件
- **SQL参数提取正则表达式**：改进以解决CI失败
- **跨平台兼容性**：增强SQL分析的健壮性
...
| **🌍 多语言支持** | 企业级语言引擎 | **Java** - 完整支持，包括Spring、JPA框架<br>**Python** - 完整支持，包括类型注解、装饰器<br>**SQL** - **🆕 增强完整支持**，包括表、视图、存储过程、函数、触发器、索引，专用数据库中心输出格式<br>**JavaScript** - 企业级支持，包括ES6+、React/Vue/Angular、JSX<br>**TypeScript** - **完整支持**，包括接口、类型、装饰器、TSX/JSX、框架检测<br>**HTML** - **🆕 完整支持**，包括DOM结构、元素分类、属性提取<br>**CSS** - **🆕 完整支持**，包括选择器分析、属性分类、样式规则<br>**Markdown** - **完整支持**，包括标题、代码块、链接、图片、表格、任务列表、引用<br>**Go** - **🆕 完整支持**，包括包、函数、方法、结构体、接口、类型别名、goroutine、channel<br>**Rust** - **🆕 完整支持**，包括模块、函数、结构体、枚举、trait、impl块、宏、async函数、生命周期<br>**Kotlin** - **🆕 完整支持**，包括包、类、数据类、密封类、对象、接口、函数、协程<br><br>**备注**: 目前15种语言具有完整的插件实现。C/C++、JSON等语言虽已定义但尚未实现。 | 框架感知解析<br>语法扩展支持<br>持续语言更新 |
