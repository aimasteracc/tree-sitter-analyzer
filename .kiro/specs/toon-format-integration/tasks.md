...
| Phase 3: CLI統合 | Week 2-3 | ❌ 未着手 | `--format toon` オプション |
| Phase 4: 最適化・ドキュメント | Week 3-4 | ❌ 未着手 | ベンチマーク、ドキュメント |
...
**サブタスク**:
- [ ] 3.1.1 `--format toon` オプション追加
- [ ] 3.1.2 `--toon-use-tabs` オプション追加
- [ ] 3.1.3 ヘルプテキスト更新
- [ ] 3.1.4 各コマンドクラス更新
...
| `cli/commands/table_command.py` | テーブル出力コマンド |
| `cli/commands/query_command.py` | クエリコマンド |
| `cli/commands/partial_read_command.py` | 部分読み取りコマンド |
| `cli/commands/list_files_cli.py` | ファイルリストCLI |
| `cli/commands/search_content_cli.py` | コンテンツ検索CLI |
| `cli/commands/find_and_grep_cli.py` | ファイル検索・grepCLI |
...
**作成予定ファイル**:
- `tests/cli/test_toon_cli.py`
- `tests/cli/test_toon_batch.py`

**_要件: 5.3, 5.4, 6.2_**

---

## Phase 4: 最適化・ドキュメント ❌ 未着手
...
**サブタスク**:
- [ ] 4.1.1 `ToonEncodeError` 例外クラス作成
- [ ] 4.1.2 循環参照検出ロジック実装
- [ ] 4.1.3 JSONフォールバック機構実装
- [ ] 4.1.4 エラーログ記録実装
- [ ] 4.1.5 エラーハンドリングテスト追加
...
**サブタスク**:
- [ ] 4.3.1 ベンチマークスイート作成
- [ ] 4.3.2 トークンカウント実装（tiktoken使用）
- [ ] 4.3.3 ベンチマークレポート生成
- [ ] 4.3.4 削減目標検証（>50%）

**作成予定ファイル**:
- `examples/calculate_toon_token_comparison.py`
- `tests/benchmarks/test_token_reduction.py`
...
**サブタスク**:
- [ ] 4.4.1 TOON フォーマットガイド作成
- [ ] 4.4.2 CLI リファレンス更新
- [ ] 4.4.3 MCP ツールドキュメント更新
- [ ] 4.4.4 移行ガイド作成
- [ ] 4.4.5 README 更新

**作成予定ファイル**:
- `docs/toon-format-guide.md`
- `docs/toon-migration-guide.md`
...
**作成予定ファイル**:
- `examples/toon_demo.py`
- `examples/toon_vs_json_demo.py`
...
| Phase 2 | 3 | 4.5日 | Week 2 |
| Phase 3 | 2 | 2.5日 | Week 2-3 |
| Phase 4 | 5 | 5.5日 | Week 3-4 |
| **合計** | **14** | **18.5日** | **~4週間** |
...
5. ⏭️ **Phase 3 開始**: タスク 3.1 (CLI引数パーシング)
6. ⏭️ GitHub Issues 作成（オプション）
7. ⏭️ 週次進捗レビュースケジュール設定
