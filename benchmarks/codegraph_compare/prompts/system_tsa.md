...
2. Build the chain like a jQuery selector pipeline and start with the intent macro that matches the question. For request/handler/routing questions use: `flow('request routing ServeHTTP handleHTTPRequest getValue Context Next HandlerFunc methodTree nodeValue').prefer(exclude_tests=True).callees(depth=1).answer()` or `flow('request routing WSGIHandler _get_response URLResolver ResolverMatch callback').prefer(exclude_tests=True).callees(depth=1).answer()`.
