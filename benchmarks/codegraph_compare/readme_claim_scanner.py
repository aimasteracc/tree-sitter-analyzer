...
_SAFE_SPANS = (
    ...
    # Re-pinned 2026-08-20: 325 -> 330 for the RFC-0027 §L7/§L8 capability
    # flags (--project-card, --plan-rename, --plan-rename-to,
    # --refactor-queue, --refactor-queue-top-n).
    # Deliberately an exact count, not \d+ — the registry must force a
    # conscious re-pin when the CLI surface changes.
    re.compile(r"\b330 CLI flags\b", re.IGNORECASE),
    re.compile(r"330 の CLI フラグ|330 个 CLI flag", re.IGNORECASE),
