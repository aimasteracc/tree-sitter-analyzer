...
- **Depends on**: [RFC-0022 Phase 0](0022-task-outcome-apis.md)
- **Normative artifacts**: [schema](schemas/edge-evidence-v1.schema.json), [golden bundle](fixtures/edge-evidence-v1-golden.json), [negative mutation corpus](fixtures/edge-evidence-v1-negative.json)
- **Implementation authorization**: none

## 1. Boundary and ownership

RFC-0022 owns `task-outcome/v1`, routing, freshness oracles, budgets, and
canonical JSON. This RFC freezes only graph-edge records. It adds no parser,
resolver, graph, cache, facade, CLI, or MCP tool.
...
3. Compare every present owner field, without normalization, to the invoked
   ...
   generated-registry entry's `(producer_rule_id,producer_rule_version)`. Any
   disagreement adds the single closed reason `OWNER_MISMATCH`. The versioned
...
5. Compare `source_endpoint.node_id` and, for selected states,
   `target_endpoint.node_id` to the corresponding fields of
   `proposed_edge_key`; any difference or unusable proposed key adds
   `TARGET_DECLARATION_MISMATCH`. This comparison is semantic, not a JSON Schema
   capability.
6. Compare `freshness_signal` and the complete snapshot tuple to authoritative
   `index.status`. `stale` or `superseded` adds `STALE_SNAPSHOT`; missing/partial
...
```json
["INCOMPATIBLE_SCHEMA","FACADE_MISSING","ACTION_MISSING","ACTION_VERSION_MISSING","RULE_ID_MISSING","RULE_VERSION_MISSING","OWNER_MISMATCH","SOURCE_LOCATOR_UNAVAILABLE","AMBIGUOUS_TARGET","UNRESOLVED_TARGET","NO_TARGET","TARGET_LOCATOR_UNAVAILABLE","PROPOSED_EDGE_KEY_MISSING","TARGET_DECLARATION_MISMATCH","FRESHNESS_SIGNAL_MISSING","SNAPSHOT_MISSING","FINGERPRINT_MISSING","PARTIAL_SNAPSHOT","STALE_SNAPSHOT","SNAPSHOT_MISMATCH","TRUNCATED","UNSUPPORTED_KIND","MALFORMED_RESULT","BUDGET_EXHAUSTED","CONTRADICTORY_EDGE_EVIDENCE"]
```
...
1. Recompute each raw observation digest from canonical bytes. Every evidence
   and provenance result hash must equal the corresponding raw digest, and each
   evidence primitive/snapshot must equal its raw observation.
2. Require every evidence `collection_id` and `provenance_id` to resolve exactly
   ...
   collection link equals that collection. Provenance input evidence refs must
   resolve. No duplicate, dangling, extra, or missing link is permitted.
3. Require collection `item_refs` to be unique and ascending by Unicode code
   ...
   non-exact totals to be null. `not_truncated` requires an exact total equal to
   returned count.
...
5. Parse each normalized request canonical string, require that reserialization
   is byte-identical, recompute its hash, and require exactly one entry for every
   distinct provenance `request_sha256`, with no extra entry.
6. Validate each diagnostic reason list against §3 priority and require
   `freshness.reason == reasons[0]`.
...
The checked-in golden bundle contains two real `calls` observations
...
`edge-evidence-v1-stable-sort-base.json` contains two evidence records with closed
reverse links in one collection, matching counts and recomputable preimages; its
sort case reverses only `item_refs`, so stable ordering is the first failure. The
corpus pins absolute/backslash/dot paths, reversed ranges, endpoint mismatch, all
five owner mismatches and missing fields, proposed-key/freshness missing, dangling
and unsorted refs, count/total/preimage/request/result mismatches, reason order and
freshness equality, and ambiguous/unresolved/no-target/stale denial.

Acceptance validates all five JSON artifacts, checks the Draft 2020-12 schema,
runs the complete schema-plus-semantic validator, recomputes every request,
result, and record ID, and proves each mutation has its expected primary reason.
It also proves no writes under `read_existing`, no direct mint, no stale positive
ID, and exact JSON/TOON decoded-model equality.
