...
2026-09-08，原子保存事件修复 #1403 已合入 `develop@989c0874`，本分支已合并
该基线。随后真实默认 polling 联调发现：watcher 直接调用旧式 `sync()`，没有
冻结候选清单，所以更新符号后仍是 incomplete，Pulse 正确拒绝读取并返回
`NO_EXACT_FULL_INDEX_MANIFEST`。这也是此前只断言符号行变化的测试遗漏。

watcher 现使用与手动同步相同的默认范围、冻结候选和认证路径，并在 finally
释放候选资源；没有放宽增量引擎的完整性门槛。回归断言新增初次同步 complete、
原子保存后 manifest 指纹与实际索引一致、默认排除范围可重放。

两文件真实默认 polling（5 秒轮询、2 秒 debounce）保存改名与调用者后：
...
Windows Python 3.11 首轮原生结果（`0ebb8b03`，job `101950285012`）：
23,024 passed、3 failed、1,188 skipped、1 xfailed，372.30 秒。此前 36 项
Pulse/CLI 失败消失，新增 10 项 Windows 原生用例没有跳过且全部通过。仍失败的
三项是旧测试入口/假设：普通未认证索引预期一律 unsupported、异常测试只替换
POSIX 扫描器、复用测试未替换现在使用的跨平台扫描入口。

相应测试现明确区分「原生能力可用但索引 partial」和「能力被禁用」，同时在
两个平台路由上验证扫描器内部 TypeError 传播，并保留精确代次与 reader pin
断言。三个文件本地 117 项通过。等待新 head 的完整 CI，不把首轮 3 项失败
改写为全绿。
