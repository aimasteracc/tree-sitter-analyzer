...
#: TOON blob. Everything else is recoverable from ``toon_content``.
...
- **Unit (Phase 1):** `compact_only=True` on `file_health` → response contains
  ...
  `queue_ledger` duplicated at top level; `success`/`verdict` still present.
...
- **Size regression:** assert `len(compact) < len(legacy_toon)` for a
  metadata-heavy tool, and `compact ≤ pure_json` (the headline win).
...
- **Phase 2 (separate gate):** golden re-baseline; assert `toon_content` and
  ...
  (bulk: `search_content`; metadata: `file_health`).

## Acceptance criteria

- [ ] `compact_only` param added to `apply_toon_format_to_response`, default `False`.
- [ ] `TOON_CONTROL_SURFACE` defined and documented.
...
- [ ] Size-regression test green (compact < legacy; compact ≤ pure JSON).
...
2. Is `TOON_CONTROL_SURFACE` the right minimal set, or should `summary_line`
   (often the single most-read line) also be retained as JSON for zero-parse
   triage?
