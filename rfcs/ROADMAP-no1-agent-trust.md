...
| [#1491](https://github.com/aimasteracc/tree-sitter-analyzer/pull/1491) 认证源码正文 | Native 已成功；抽取后 CI 指导仍引用已失效的源码检查，公开 execute guidance 测试修复已于 2026-09-15 UTC 以 `b638cb61` 提交并推送，52 项焦点测试通过。Windows 正文缺失已定位为 `CertifiedIndexRead.read_source` 无条件调用仅支持 POSIX 的 `source_oracle.safe_workspace_path`，该路径按设计拒绝 Windows；有界 Windows `NativeFiles` 生产修复因涉及安全敏感的文件系统与句柄改动，被自动审批以“监控请求未明确授权”为由阻断，所有生产改动已回退，须等待知情授权，不能视为正在实现 | 修复指导与 Windows 读取缺口，重跑相关焦点测试、补丁覆盖和原生 CI；最终 head 经审查后方可合入 |
