...
- [x] **T1** Write test: `DependencyGraph.has_node()` returns True for existing node, False otherwise
...
- [x] **I1** Add `has_node()`, `node_count()`, `edge_count()` to `DependencyGraph`
  - File: `tree_sitter_analyzer/project_graph.py` — after `edges()` method
- [x] **I2** Migrate call sites in production code (4 files, ~16 call sites)
  ...
  - Also fixed `CachedDependencyGraph` (duck-typed wrapper) and `FakeGraph` test stub

## Phase 3 — Verification

- [x] **V1** Tests pass (focused: test_project_graph, test_dependency_analysis_tool, etc.)
- [x] **V2** Run `uv run python -m tree_sitter_analyzer --change-impact --format json`
- [x] **V3** Push + PR to develop (#202)
